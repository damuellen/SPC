...
let flags = [
  "-cross-module-optimization", "-Ounchecked", "-enforce-exclusivity=unchecked", "-DRELEASE",
]
...
var package = Package(
  name: "SPC", platforms: [.macOS(.v10_13), .iOS(.v12)],
  products: [
    ...
    .executable(name: "PinchPointTool", targets: ["PinchPointTool"]),  // .library(name: "BlackBoxModel", type: .dynamic, targets: ["BlackBoxModel"]),
  ],
  dependencies: [
    .package(
      url: "https://github.com/apple/swift-argument-parser.git", .upToNextMinor(from: "0.5.0")),
    ...
    .package(
      ...
      .upToNextMajor(from: "1.5.0")),// .package(url: "https://github.com/pvieito/PythonKit.git", .branch("master")),
    // .package(url: "https://github.com/jpsim/Yams.git", from: "4.0.1")
  ],
  targets: [
    .target(name: "Libc"), .target(name: "Helpers", dependencies: ["Libc"], swiftSettings: swift),
    .target(
      name: "PhysicalQuantities", dependencies: ["Config", "Meteo", "Libc", "Helpers"],
      swiftSettings: swift),
    .executableTarget(name: "TransTES", dependencies: ["Helpers"], swiftSettings: swift),
    .executableTarget(
      name: "SunOl",
      dependencies: [
        "Helpers", "PhysicalQuantities", .product(name: "xlsxwriter", package: "xlsxwriter.swift"),
        .byName(name: "Swifter", condition: .when(platforms: [.linux, .macOS])),
      ], swiftSettings: swift, linkerSettings: [.unsafeFlags(linker, .when(platforms: [.windows]))]),
    .target(
      name: "PinchPoint", dependencies: ["CPikchr", "CIAPWSIF97", "Helpers", "PhysicalQuantities"],
      swiftSettings: swift),
    .executableTarget(
      name: "PinchPointTool",
      dependencies: [
    ...
    .target(
      name: "SolarPosition", dependencies: ["DateGenerator", "CSOLPOS", "CSPA"],
      swiftSettings: swift),
    .target(
      name: "BlackBoxModel",
      dependencies: [
      ...
      linkerSettings: [
        .linkedLibrary("sqlite3.lib", .when(platforms: [.windows])),
        .unsafeFlags(linker, .when(platforms: [.windows])),
      ]),
    .target(
      name: "ThermalStorage", dependencies: ["Libc", "Helpers", "PhysicalQuantities"],
      swiftSettings: swift),
    ...
    .target(name: "Meteo", dependencies: ["DateGenerator", "SolarPosition"], swiftSettings: swift),
    .executableTarget(
    ...
    //   swiftSettings: swift),
    .testTarget(name: "MeteoTests", dependencies: ["DateGenerator", "SolarPosition", "Meteo"]),
