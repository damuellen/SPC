...
class StorageTests: XCTestCase {
  func testsParasitics() {
    ...
    plant.heatFlow.demand.megaWatt = min(
      (demand / 0.39), HeatExchanger.parameter.heatFlowHTF)

    plant.heatFlow = storage.demandStrategy(
      powerBlock: &powerBlock, heatFlow: plant.heatFlow)
    _ = Storage.perform(
      storage: &storage, solarField: &solarField, steamTurbine: &steamTurbine,
      powerBlock: &powerBlock, heatFlow: &plant.heatFlow)
    ...
    _ =
      storage.massFlow.rate * SolarField.parameter.HTF.heatContent(storage)
      / 1_000
