...
class CollectorTests: XCTestCase {
  func testsEfficiency() {
    ...
    var collector = Collector(
      parabolicElevation: 63.96, theta: 29.18, cosTheta: 0.87, efficiency: 0,
      insolationAbsorber: 0)
    collector.efficiency(ws: 0)
    XCTAssertEqual(collector.efficiency, 0.72, accuracy: 0.1)
    collector = Collector(
      parabolicElevation: 51.49, theta: 55.53, cosTheta: 0.56, efficiency: 0,
      insolationAbsorber: 0)
    collector.efficiency(ws: 0)
  ...
  func testMean() {
    guard
      let file = try? MeteoDataFileHandler(
        forReadingAtPath: "/Users/daniel/spc/COM/Tunol.mto"),
      let meteo = try? file.data(valuesPerHour: 12),
      let location = try? file.metadata().location
    else { return }
    let sun = SolarPosition(
      coords: location.coordinates, tz: -1, year: 2017, frequence: .fiveMinutes
    )
    let numberOfSCAsInRow = Double(SolarField.parameter.numberOfSCAsInRow)
    let edge1 =
      SolarField.parameter.distanceSCA / 2 * (1 - 1 / numberOfSCAsInRow)
    ...
    for x in zip(zip(dates, meteo), values1.indices) {
      ...
      print(
        DateTime(x.0.0).description, values12[x.1], values22[x.1],
        values1[x.1], values2[x.1], dni, dni * values1[x.1] * values2[x.1],
        to: &out)
    }
  ...
  func testsTracking() {
    ...
    let januaryAM = SolarPosition.Output(
      ...
      declination: -23.023, incidence: 0.442,
      cosIncidence: cos(0.442 * .pi / 18), sunrise: 435.029, sunset: 1005.108)

    let aprilAM = SolarPosition.Output(
      zenith: 38.333, azimuth: 141.835, elevation: 51.666, hourAngle: -22.626,
      declination: 4.810, incidence: 0.784,
      cosIncidence: cos(0.784 * .pi / 18), sunrise: 345.913, sunset: 1095.098)

    let julyAM = SolarPosition.Output(
      zenith: 35.192, azimuth: 102.700, elevation: 54.807, hourAngle: -37.677,
      declination: 23.058, incidence: 0.817,
      cosIncidence: cos(0.817 * .pi / 18), sunrise: 285.618, sunset: 1155.803)

    let julyPM = SolarPosition.Output(
      zenith: 34.916, azimuth: 256.965, elevation: 55.083, hourAngle: 37.312,
      declination: 23.043, incidence: 0.819,
      cosIncidence: cos(0.819 * .pi / 18), sunrise: 285.714, sunset: 1155.786)

    let octoberPM = SolarPosition.Output(
      zenith: 55.618, azimuth: 232.403, elevation: 34.381, hourAngle: 40.944,
      declination: -3.520, incidence: 0.564,
      cosIncidence: cos(0.564 * .pi / 18), sunrise: 356.8873, sunset: 1055.5590
    )
