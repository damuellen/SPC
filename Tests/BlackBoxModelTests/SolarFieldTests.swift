...
class SolarFieldTests: XCTestCase {
  func testsSingleAxisTracker() {
    ...
    for i in expected.indices {
      let angles = singleAxisTracker(
        apparentZenith: zenith[i], apparentAzimuth: azimuth[i], latitude: 1.0,
        maxAngle: 55.0, GCR: 0.444)
  ...
  func testsOutletTemperature() {
    ...
    solarField.temperature.inlet = Temperature(
      celsius: SolarField.parameter.designTemperatureInlet)
    solarField.temperature.outlet = Temperature(
      celsius: SolarField.parameter.designTemperatureOutlet)
    ...
    solarField.calculate(
      collector: collector, ambient: Temperature(celsius: 25.0))
