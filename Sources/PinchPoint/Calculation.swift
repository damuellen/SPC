...
public struct Calculation: Codable {
  public init(parameter: HeatExchangerParameter) { self.parameter = parameter }
  ...
  public var turbine = WaterSteam(
    temperature: .init(celsius: 380.0), pressure: 102.85, massFlow: 66.42)
  ...
  public var reheatInlet = WaterSteam(
    temperature: .init(celsius: 217.8), pressure: 22.23, massFlow: 53.58,
    enthalpy: 2800.9)
  ...
  mutating func evaporation() -> (Double, Double, Double) {
    ...
    let pressureDropTotal =
      turbine.pressure + pd.steamGenerator_superHeater + pd.superHeater
      + pd.superHeater_turbine

    steamGenerator.pressure.ws.outlet = pressureDropTotal

    let enthalpyBeforeEvaporation = WaterSteam.enthalpyLiquid(
      pressure: pressureDropTotal)

    let enthalpyAfterEvaporation = WaterSteam.enthalpyVapor(
      pressure: pressureDropTotal)
    ...
    let enthalpyChangeDueToEvaporation =
      enthalpyAfterEvaporation - enthalpyBeforeEvaporation
    let evaporationPower =
      enthalpyChangeDueToEvaporation * turbine.massFlow / 1_000

    let enthalpyChangeDueToSuperHeating = enthalpy - enthalpyAfterEvaporation
    let superHeatingPower =
      enthalpyChangeDueToSuperHeating * turbine.massFlow / 1_000
  ...
  mutating func powerSteamGenerator() -> Double {
    ...
    let powerForWaterHeatingInsideSg =
      waterEnthalpyChangeDueToPreheating * economizer.massFlow.ws.outlet
      / 1_000
  ...
  mutating func preheat() -> Double {
    ...
    economizer.power =
      economizer.wsEnthalpyChange * economizer.massFlow.ws.outlet / 1_000  //

    let requiredHTFEnthalpyChange =
      economizer.power / economizer.massFlow.htf * 1_000

    economizer.enthalpy.htf.inlet = HTF.enthalpy(
      economizer.temperature.htf.inlet)
    let requiredHTFEnthalpyOutlet =
      economizer.enthalpy.htf.inlet - requiredHTFEnthalpyChange

    economizer.enthalpy.htf.outlet = requiredHTFEnthalpyOutlet
    economizer.temperature.htf.outlet = HTF.temperature(
      economizer.enthalpy.htf.outlet)
  ...
  mutating func reheat() -> Double {
    ...
    reheater.enthalpy.htf.outlet = HTF.enthalpy(
      reheater.temperature.htf.outlet)
  ...
  public mutating func callAsFunction() {
    ...
    reheater.power =
      reheater.massFlow.ws.outlet * reheater.wsEnthalpyChange / 1_000
    ...
    steamGenerator.temperature.ws.inlet =
      ...
      - parameter.temperatureDifferenceWater
    ...
    superheater.pressure.ws.outlet =
      turbine.pressure + pressureDrop.superHeater_turbine
    superheater.massFlow.ws.inlet = turbine.massFlow
    superheater.massFlow.ws.outlet = superheater.massFlow.ws.inlet
    superheater.pressure.ws.inlet =
      superheater.pressure.ws.outlet + pressureDrop.superHeater
    ...
    let superheaterSaturatedSteamEnthalpyOutletVirtual =
      WaterSteam.enthalpyVapor(
    ...
    let superheaterSteamQualityOutlet =
      ...
      / (superheaterSaturatedSteamEnthalpyOutletVirtual
        - superheaterWaterEnthalpyOutletVirtual)
    ...
    superheater.power =
      enthalpyChangeDueToSuperheatingSteam * turbine.massFlow / 1_000

    superheater.temperature.htf.inlet = upperHTFTemperature
    superheater.enthalpy.htf.inlet = HTF.enthalpy(
      superheater.temperature.htf.inlet)

    let saturatedSteam = WaterSteam.temperature(
      pressure: steamGenerator.pressure.ws.outlet)

    steamGenerator.temperature.htf.outlet =
      saturatedSteam + parameter.temperatureDifferenceHTF
    economizer.temperature.htf.inlet = steamGenerator.temperature.htf.outlet
    steamGenerator.enthalpy.htf.outlet = HTF.enthalpy(
      steamGenerator.temperature.htf.outlet)
    ...
    superheater.temperature.htf.outlet = HTF.temperature(
      superheater.enthalpy.htf.outlet)

    steamGenerator.temperature.htf.inlet = superheater.temperature.htf.outlet
    steamGenerator.enthalpy.htf.inlet = HTF.enthalpy(
      steamGenerator.temperature.htf.inlet)
    ...
    let mixHTFAbsoluteHeatFlow =
      economizerHTFAbsoluteHeatFlowOutlet + reheatHTFAbsoluteHeatFlowOutlet
