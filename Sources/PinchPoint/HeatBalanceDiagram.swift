...
public struct HeatBalanceDiagram {
  public init(values: Calculation) {
    self.streams = [
      Stream(
        values.mixHTFTemperature, 0, values.mixHTFMassflow,
        values.mixHTFAbsoluteEnthalpy), values.economizer.htfSide.outlet,
      values.economizer.steamSide.inlet, values.economizer.steamSide.outlet,
      ...
      values.reheater.steamSide.inlet, values.reheater.steamSide.outlet,
      values.reheater.htfSide.inlet, values.superheater.steamSide.outlet,
      values.superheater.htfSide.outlet, values.steamGenerator.htfSide.outlet,
      values.reheater.htfSide.outlet, values.superheater.htfSide.inlet,
      Stream(
        values.upperHTFTemperature, 0, values.mixHTFMassflow,
        values.superheater.enthalpy.htf.inlet),
    ]
    self.singleValues = [
      ...
      ("LMTD", values.economizer.LMTD), ("LMTD", values.steamGenerator.LMTD),
      ("Blow Down", String(format: "%.2f kg/s", values.blowDownMassFlow)),
      ("LMTD", values.superheater.LMTD), ("LMTD", values.reheater.LMTD),
    ]
  }
  ...
  struct Stream {
    ...
    init(
      _ temperature: Temperature, _ pressure: Double, _ massFlow: Double,
      _ enthalpy: Double
    ) {
  ...
  public var svg: String {
    let diagram = """
      ...
      circle "G" big big
      """
    let svg = diagram.withCString {
      String(cString: pikchr($0, "c", 0, nil, nil))
    }
    return svg
  }
}
