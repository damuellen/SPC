...
struct LocationInfo: ParsableArguments {
  @Option(name: [.customShort("z"), .long], help: "Time zone") var timezone:
    Int?
  @Option(
  ...
  @Option(
    ...
    help: "Latitude (decimal degrees)") var latitude: Double?
  @Option(
    name: [.customLong("ele", withSingleDash: true)],
    help: "Elevation (meters)") var elevation: Double?
...
struct SolarPerformanceCalculator: ParsableCommand {
  ...
  @Option(
    name: .shortAndLong,
    help: "Custom name, otherwise they are numbered with 2 digits.")
  var resultName: String?
  @Option(name: .shortAndLong, help: "Year of simulation.") var year: Int?
  @OptionGroup() var location: LocationInfo
  ...
  @Flag(help: "Output performance data as sqlite db.") var database: Bool =
    false
  @Flag(name: .shortAndLong, help: "Detail overview of the model parameter.")
  var verbose: Bool = false
  @Flag(help: "Save the model parameter in json file format.") var json: Bool =
    false
  @Flag(help: "Output performance data as excel file.") var excel: Bool = false
  @Flag(help: "Open result file after calculation.") var open: Bool = false
  ...
  func run() throws {
    ...
    do {
      ...
      try BlackBoxModel.configure(meteoFilePath: path)
    } catch {
      #if os(Windows)
      if case MeteoDataFileError.fileNotFound = error {
        ...
        do { try BlackBoxModel.configure(meteoFilePath: path) } catch {
          MessageBox(
            text: (error as! MeteoDataFileError).description, caption: name)
          return
        }
      } else {
        MessageBox(
    ...
    let recording = Historian(
      name: resultName, path: pathForResult, mode: mode)
    ...
    if http {
      ...
      server.start()
      print(
        "web server listening on port \(server.port). Press Crtl+C to shut down."
      )
  ...
  static var configuration = CommandConfiguration(
    ...
    abstract:
      "Calculates the annual production of a solar thermal power plant.")
  ...
  func plotter(_ result: Recording) {
    ...
    for i in 1...365 {
      ...
      let plot = TimeSeriesPlot(
        y1: y1, y2: y2, range: day, yRange: yRange, style: .impulses)
      plot.y1Titles = ["solarfield", "powerblock", "storage"]
      plot.y2Titles = [
        "solar", "production", "toStorage", "fromStorage", "gross", "net",
        "consum",
      ]
...
extension Recording {
  ...
  func respond(request: HTTP.Request) -> HTTP.Response {
    ...
    let plot = TimeSeriesPlot(
      y1: y1, y2: y2, range: range, yRange: yRange, style: .impulses)
    ...
    guard
      let base64PNG = try? plot.callAsFunction(toFile: "")?
        .base64EncodedString()
    ...
    let table = zip(sums.map(\.description), p)
      .map { "<th>" + $0.0 + "</th><td>" + $0.1 + "</td>" }
      .joined(separator: " ")
