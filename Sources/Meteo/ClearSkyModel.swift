...
extension MeteoData {
  ...
  public static func using(
    _ sun: SolarPosition, model: ClearSkyModel = .constant,
    clouds: Bool = false
  ) -> [MeteoData] {
    ...
    for d in DateSeries(year: sun.year, interval: sun.frequence) {
      ...
      if let pos = sun[d], pos.zenith < 90 {
        ...
        let dni =
          calculate(zenith: pos.zenith, day: day, model: model)
...
private func calculate(zenith: Double, day: Int, model: ClearSkyModel)
  -> Double
{
  ...
  case .constant: dni = dni_des / S0
  case .special:
    dni =
      (0.5 * ((1 - 0.14 * al) * exp(-0.357 / pow(cz, 0.678)) + 0.14 * al)
        + (0.4237 - 0.00821 * pow(6.0 - al, 2)
          + (0.5055 + 0.00595 * pow(6.5 - al, 2))
            * exp(-(0.2711 + 0.01858 * pow(2.5 - al, 2)) / (cz + 0.00001))))
      / 1.45
