...
public class MeteoDataSource {
    ...
    -> MeteoDataSource
  {
    ...
    for d in DateGenerator(year: sun.year, interval: sun.frequence) {
      step += 1
      if step == steps { day += 1; step = 0 }
      if let pos = sun[d], pos.zenith < 90 {
        if (step * 2) % steps == 0 {
          isCloudy = (rng.random() < 0.314) && clouds
        }
        let dni = Float(insolation(zenith: pos.zenith, day: day)
...
fileprivate func insolation(
  zenith: Double, day: Int, model: ClearSkyModel = .hottel) -> Double
{
  ...
  case .hottel:
    dni = 0.4237 - 0.00821 * pow(6.0 - al, 2)
  ...
  case .moon:
    let dpres = 1.0
    let del_h2o = 1.0
    dni = 1 - 0.263 * ((del_h2o + 2.72) / (del_h2o + 5))
