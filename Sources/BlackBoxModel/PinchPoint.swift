...
public func foo() -> Double {
	...
	let superheaterLiveSteamEnthalpy = WaterSteam.enthalpy(p: liveSteamPressureAtTurbineInlet, t: Temperature(celsius: liveSteamTemperatureAtTurbineInlet))
	...
	let reheatOutletSteamEnthalpy = WaterSteam.enthalpy(p: reheatOutletSteamPressure, t: Temperature(celsius: reheatOutletSteamTemperature))
	...
	let economizerHtfAbsoluteHeatFlowOutlet = economizerHtfEnthalpyOutlet * economizerHtfMassflow / 1_000
	...
	let ReheaterPower = reheatInletSteamMassflow * (reheatOutletSteamEnthalpy - reheatInletSteamEnthalpy) / 1_000
	...
	let reheatHtfTemperatureOutlet = reheatSteamTemperatureInlet + reheatOil_Outlet_SteamInletTemperatureDifference
	...
	let reheatHtfEnthalpyInlet = SolarField.parameter.HTF.enthalpy(Temperature(celsius: reheatHtfTemperatureInlet))
	let reheatHtfEnthaplyOutlet = SolarField.parameter.HTF.enthalpy(Temperature(celsius: reheatHtfTemperatureOutlet))
