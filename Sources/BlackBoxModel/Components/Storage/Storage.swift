...
public struct Storage: Component {
  ...
  public struct PerformanceData: HeatFlow {
  ...
  static let initialState = PerformanceData(
    ...
    temperature: (0.0, 0.0), temperatureTanks: (566.0, 666.0),
  ...
  private static func parasitics(storage: inout PerformanceData) -> Double {
  ...
  static func update(_ status: inout Plant.PerformanceData,
    ...
    case .charging: // heat can be stored
      ...
      if parameter.tempInCst[1] > 0 { // usually = 0
        if status.storage.heatrel < 0.5 {
          Properties.StoFit = 1
        } else {
          for i in 0 ..< 3 {
            Properties.StoFit += parameter.tempInCst[i] * status.storage.heatrel ** Double(i)
          }
        }
        ...
        if status.storage.heatrel < 0 {
          Properties.StoFit += parameter.tempInCst[0]
            - (parameter.designTemperature.cold - status.storage.temperatureTanks.cold).kelvin
        } else {
      ...
      if parameter.heatExchangerRestrictedMax,
        ...
        if case .demand = parameter.strategy { // (always)
          ...
          thermal.dump += thermal.production
            - HeatExchanger.parameter.SCCHTFthermal + thermal.storage
        } else {
        ...
        thermal.solar = -status.solarField.thermal(fluid: htf)
    ...
    case .fc: // heat can be stored
      ...
      status.storage.temperature.outlet = Temperature(Properties.StoFit)
      thermal.storage = -status.storage.thermal(fluid: htf) // [MW]
    ...
    case .ex: // heat can be released
      ...
      if time.hour >= Properties.SSetTime && time.hour < (Properties.SSetTime + 1) && parameter.isVariable {
        switch parameter.definedBy {
        case .hours:
          status.storage.heatStored = status.storage.heatrel * Design.layout.storage
            * SteamTurbine.parameter.power.max / SteamTurbine.parameter.efficiencyNominal
        case .cap:
          status.storage.heatStored = status.storage.heatrel * Design.layout.storage_cap
        case .ton:
          status.storage.hSalt.hot = salt.heatCapacity[0]
            * (parameter.designTemperature.hot.celsius) + 0.5 * salt.heatCapacity[1]
            * (parameter.designTemperature.hot.celsius) ** 2 - 350.5536
          
          status.storage.hSalt.cold = salt.heatCapacity[0]
            * (parameter.designTemperature.cold.celsius) + 0.5 * salt.heatCapacity[1]
            * (parameter.designTemperature.cold.celsius) ** 2 - 350.5536
          
          status.storage.heatStored = status.storage.heatrel * Design.layout.storage_ton
      ...
      if Properties.QoutLoad == 0 && parameter.isVariable {
        ...
        Properties.QoutLoad = status.storage.heatStored / nightHour / (SteamTurbine.parameter.power.max
          / SteamTurbine.parameter.efficiencyNominal)
      ...
      if parameter.heatExchangerRestrictedMax,
        ...
        status.storage.massFlow = MassFlow(thermal.storage /
          htf.heatDelta(status.storage.temperature.outlet,
                        status.storage.temperature.inlet) * 1_000)
        // go StorageOutletTemp
        thermal.storage = -status.storage.thermal(fluid: htf)
      }
    ...
    case .freezeProtection:
      ...
      status.storage.massFlow = SolarField.parameter.antiFreezeFlow
        .adjusted(with: splitfactor)
      
      status.solarField.header.massFlow = SolarField.parameter.antiFreezeFlow
      ...
      if parameter.tempInCst[1] > 0 {
        ...
        if parameter.tempExCst.indices.contains(2) {
          ...
          if status.storage.heatrel > 0.5 {
            ...
            for i in 0 ..< 3 {
              Properties.StoFit += parameter.tempExC0to1[i] * status.storage.heatrel ** Double(i)
            }
          }
    ...
    if parameter.tempInCst[1] > 0 { // it doesn//t get in here usually.
      if parameter.tempInCst[2] > 0 {
       ...
       //   }
        ...
        status.storage.heatLossStorage = Properties.StoFit * 3_600 * 0.0000001 * Design.layout.storage // [MW] !!!!
      } else {
    ...
    // StorageOutletTemp
      ...
      Properties.StoFit *= parameter.designTemperature.hot.kelvin - parameter.tempExCst[1]
    } else {
      Properties.StoFit = Properties.StoFit.toKelvin
      if status.storage.heatrel < 0 {
        Properties.StoFit += parameter.tempExCst[0]
          - (parameter.designTemperature.hot.kelvin - status.storage.temperatureTanks.hot.kelvin)
      } else {
  ...
  static func calculate(storage: inout Storage.PerformanceData,
    ...
    case .hours:
      ...
      storage.massSalt *= HeatExchanger.parameter.SCCHTFthermal * 1_000 * 3_600
        / (storage.hSalt.hot - storage.hSalt.cold)
    case .cap:
    ...
    if parameter.tempInCst[1] > 0 { // it doesnt get in here usually, therefore not updated yet
      ...
      storage.heatrel = storage.heatStored
        / (Design.layout.storage * HeatExchanger.parameter.SCCHTFthermal)
    } else {
      ...
      case .charging:
        ...
        storage.hSalt.cold = salt.heatCapacity[0]
          * (storage.temperatureTanks.cold.celsius)
          + 0.5 * salt.heatCapacity[1]
          * (storage.temperatureTanks.cold.celsius) ** 2 - 350.5536
      ...
      case .ex:
        storage.hSalt.hot = salt.heatCapacity[0]
          * (storage.temperatureTanks.hot.celsius)
          + 0.5 * salt.heatCapacity[1]
          * (storage.temperatureTanks.hot.celsius) ** 2 - 350.5536
      ...
      if steamTurbine.isMaintained {
        ...
        if storage.mass.hot > 0 { // parameter.dischargeToTurbine * Saltmass {
          ...
          storage.temperatureTanks.hot = Temperature((-salt.heatCapacity[0]
        ...
        if storage.mass.cold > parameter.dischargeToTurbine * storage.massSalt {
          ...
          // temp after cool down
          storage.temperatureTanks.cold = Temperature((-salt.heatCapacity[0]
            + (salt.heatCapacity[0] ** 2
              - 4 * (salt.heatCapacity[1] * 0.5)
  ...
  static func update(_ status: inout Plant.PerformanceData,
    ...
    if Design.hasGasTurbine {
      status.powerBlock.massFlow = MassFlow(HeatExchanger.parameter.SCCHTFthermal /
        (htf.heatDelta(
          HeatExchanger.parameter.temperature.htf.inlet.max,
          HeatExchanger.parameter.temperature.htf.outlet.max) / 1_000))
      thermalDiff = thermal.production - HeatExchanger.parameter.SCCHTFthermal
    } else {
      ...
      if case .demand = parameter.strategy {
        ...
        if status.powerBlock.massFlow < 0.0 { // to avoid negative massfows
          ...
          thermalDiff = thermal.production - HeatExchanger.parameter.SCCHTFthermal
          // changed back as thermal.production - HeatExchanger.parameter.SCCHTFheat// thermal.demand     // [MW]
          if parameter.heatExchangerRestrictedMin {
            // added to avoid input to storage lower than minimal HX// s capacity
            thermal.toStorageMin = HeatExchanger.parameter.SCCHTFthermal
              * (1 - parameter.massFlow.rate / SolarField.parameter.massFlow.max.rate)
              / (parameter.massFlow.rate / SolarField.parameter.massFlow.max.rate)
            
            if thermalDiff > 0 && thermalDiff < thermal.toStorageMin {
              status.powerBlock.massFlow = MassFlow((HeatExchanger.parameter.SCCHTFthermal
                - (thermal.toStorageMin - thermalDiff))
                / (htf.heatDelta(
                  HeatExchanger.parameter.temperature.htf.inlet.max,
                  HeatExchanger.parameter.temperature.htf.outlet.max) / 1_000))
              thermalDiff = thermal.toStorageMin
      ...
      if case .shifter = parameter.strategy { // parameter.strategy = "Ful" // Booster or Shifter
        ...
        if thermal.production > 0 { // Qsol > 0
          ...
          if thermal.production < thermal.demand,
            status.storage.heatrel < parameter.chargeTo,
            time.hour < 17 {
            ...
            thermalDiff = thermal.production - (status.storage.heatProductionLoad * thermal.demand)
            // TES gets the rest available
            if parameter.heatExchangerRestrictedMax {
          ...
          if case .hours = parameter.definedBy {
            ...
            if thermal.production > thermal.demand,
              status.storage.heatrel > 1 * SteamTurbine.parameter.power.max
                / SteamTurbine.parameter.efficiencyNominal / Design.layout.storage {
              i = -1
              //   } else if parameter.definedBy = "cap" {
              //       if thermal.production > thermal.demand && storage.heatrel > 1 * SteamTurbine.parameter.power.max / SteamTurbine.parameter.efficiencyNominal / Design.layout.storage_cap { i = -1
              //   } else if parameter.definedBy = "ton" {
              //       if thermal.production > thermal.demand && storage.heatrel > 1 * SteamTurbine.parameter.power.max / SteamTurbine.parameter.efficiencyNominal / Design.layout.storage_ton { i = -1
            }
            if i == -1 {
              thermal.demand = SteamTurbine.parameter.power.max
    ...
    if thermalDiff > 0 {
      ...
      // Energy surplus
      if status.storage.heatrel < parameter.chargeTo,
        status.solarField.header.massFlow >= status.powerBlock.massFlow {
      ...
      if peakTariff, noFreezeProtection,
        status.storage.heatrel > parameter.dischargeToTurbine,
        ...
        if (parameter.FC == 0 && status.collector.parabolicElevation < 0.011
          ...
          if Fuelmode != "predefined" {
            ...
            status.storage.heatrel > -0.35 && parameter.FP == 0 {
            
            Storage.update(&status, mode: .freezeProtection, thermal: &thermal)
            //Storage.update(mode: .freezeProtection, date: heatdiff, thermal: powerBlock.massFlow, status: thermal.storage)
          } else {
            Storage.update(&status, mode: .noOperation, thermal: &thermal)
            //Storage.update(mode: .noOperation, date: heatdiff, thermal: powerBlock.massFlow, status: thermal.storage)
          }
        }
      }
      ...
      case .ex:
        status.storage.hSalt.hot = salt.heatCapacity[0]
          * (status.storage.temperatureTanks.hot.celsius)
          + 0.5 * salt.heatCapacity[1]
          * (status.storage.temperatureTanks.hot.celsius) ** 2 - 350.5536
        
        status.storage.hSalt.cold = salt.heatCapacity[0]
          * ((status.storage.temperature.inlet
          + status.storage.dTHTF_ColdSalt).celsius)
          + 0.5 * salt.heatCapacity[1] * (status.storage.temperature.inlet.kelvin
            + status.storage.dTHTF_ColdSalt.celsius) ** 2 - 350.5536
      ...
      case .charging:
        status.storage.hSalt.hot = salt.heatCapacity[0]
          * (status.storage.temperature.inlet.kelvin
            - status.storage.dTHTF_HotSalt.celsius) + 0.5 // FIXME
          * salt.heatCapacity[1] * (status.storage.temperature.inlet.kelvin
            - status.storage.dTHTF_HotSalt.celsius) ** 2 - 350.5536 //
        
        status.storage.hSalt.cold = salt.heatCapacity[0]
          * (status.storage.temperatureTanks.cold.celsius)
          + 0.5 * salt.heatCapacity[1]
          * (status.storage.temperatureTanks.cold.celsius) ** 2 - 350.5536
