...
extension Storage {
  ...
  public enum TypeDir: String, Codable { case indirect, direct }
...
public enum Strategy: String, Codable {
  ...
  init?(string: String) { self.init(rawValue: string.lowercased()) }
}
...
extension Storage.Parameter: CustomStringConvertible {
  ...
  public var description: String {
    "Description:" * (name + " " + type.rawValue)
      ...
      + "Discharge Storage directly to Turbine down to Load:"
      * dischargeToTurbine.description
      + "Discharge Storage indirectly to Heater down to Load:"
      * dischargeToHeater.description
      + "Discharge Storage, if (Qsolarfield - Qdemand) <"
      * "\(heatdiff * 100) % of Qdemand"
      + "Stepwidth of Qdemand to iterate Tmin for Turbine:"
      * stepSizeIteration.description
      + "Relative Filling of Storage at Program Start (only for Thermocline):"
      * heatStoredrel.description
      ...
      * String(format: "%G", heatlossC0to1[1])
      ...
      + "Pressure Loss at Design Point [Pa]:"
      * String(format: "%G", pressureLoss)
      ...
      * prefChargeToTurbine.description + "Definition of Summer from Month:"
      * exception.lowerBound.description + "                       to Month:"
      * exception.upperBound.description + "DNI for Bad Days Winter [kWh/m2]:"
      * badDNIwinter.description + "DNI for Bad Days Summer [kWh/m2]:"
      * badDNIsummer.description
      ...
      * heatProductionLoadSummer.percentage.description
      + "Time to begin TES Discharge in Winter [hr]:"
      * dischargeWinter.description
      + "Time to begin TES Discharge in Summer [hr]:"
      * dischargeSummer.description
  }
}
...
extension Storage.Parameter: TextConfigInitializable {
  ...
  init(file: TextConfigFile) throws {
    ...
    fossilChargingTime = try [
      l2(180), l2(181), l2(182), l2(183), l2(222), l2(223), l2(224), l2(225),
    ]
...
public enum StorageMedium: String, Codable {
  ...
  static var ss = HeatTransferFluid(
    name: "Solar Salt", freezeTemperature: 240.0,
    heatCapacity: [1.44657, 0.000171715], dens: [1969.9, -0.603505, 0],
    visco: [0.0175373, -7.01716e-05, 7.62774e-08],
    thermCon: [0.44152, 0.00019, 0], maxTemperature: 400.0, h_T: [], T_h: [],
    useEnthalpy: false)

  /// Get the heat transfer fluid properties for a specific storage medium.
  var properties: HeatTransferFluid {
    switch self {
    case .solarSalt: return StorageMedium.ss
    default: fatalError()
    }
  }
}
