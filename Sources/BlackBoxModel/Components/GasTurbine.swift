...
struct GasTurbine: Parameterizable {
  ...
  static func efficiency(at load: Ratio) -> Double {
    ...
    let efficiency =
      parameter.efficiencyFromLoad(load) * parameter.efficiencyISO
  ...
  static func perform(
    ...
    boiler: Boiler.OperationMode, gasTurbine: inout GasTurbine,
    heatExchanger: HeatExchanger, steamTurbine: inout SteamTurbine,
    temperature: Temperature, fuel: Double, plant: inout Plant
  ) -> Double {
    var supply = 0.0
    ...
    demand -=
      plant.heatFlow.production.megaWatt * HeatExchanger.parameter.efficiency
    ...
    if supply == 0 {  // Ucase$(OpRCCmode(month, time.Tariff)) = "F" || Ucase$(OpRCCmode(month, time.Tariff)) = "L" {
      ...
      if case .pure = WasteHeatRecovery.parameter.operation {
        ...
        if demand < Design.layout.gasTurbine {
          ...
          // to correctDC
          let htfShare =
            demand * (1 / parameter.efficiencyISO - 1)
            ...
            production * HeatExchanger.parameter.efficiency > htfShare
          {
            ...
            demand =
              (plant.electricity.demand - SteamTurbine.parameter.efficiencySCC
                * plant.heatFlow.solar.megaWatt
          ...
          if (plant.electricity.demand - demand)
            > SteamTurbine.parameter.power.max
          {

            demand =
              (SteamTurbine.parameter.power.max
                / SteamTurbine.parameter.efficiencySCC - plant.heatFlow.solar
                .megaWatt * HeatExchanger.parameter.efficiency)
              / (WasteHeatRecovery.parameter.efficiencyNominal
      ...
      plant.heatFlow.demand.megaWatt =
        steamTurbine.load.quotient * SteamTurbine.parameter.power.max
        / efficiency.quotient
