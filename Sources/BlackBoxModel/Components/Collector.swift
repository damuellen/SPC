...
struct Collector: Parameterizable, CustomStringConvertible {
  ...
  static let initialState = Collector(
    parabolicElevation: 0, theta: 0, cosTheta: 0, efficiency: 0.0,
  ...
  public mutating func efficiency(ws: Double) {
    ...
    var opticalEfficiency =
      parameter.opticalEfficiency
    ...
    switch shadlength {
    case _ where shadlength <= parameter.extensionHCE: edge = 1
    case _ where shadlength <= solarField.distanceSCA:
      edge = 1 - (shadlength - parameter.extensionHCE) / parameter.lengthSCA
    default:
      edge =
        1 - solarField.edgeFactor[0] - (shadlength - parameter.extensionHCE)
        * solarField.edgeFactor[1]
    }

    var shadingSCA =
      abs(sin(parabolicElevation.toRadians)) * solarField.rowDistance
      / parameter.aperture
    shadingSCA = min(1, shadingSCA)
    if shadingSCA < 0.01 { shadingSCA = 1 }
    /// Angle of wind attack
    let AW: Double
    let direction = 1
    if direction < 180 {
      AW = parabolicElevation
    } else {
      AW = 180 - parabolicElevation
    }
    ...
    let T_R =
      -(939_549e-10 * parameter.lengthSCA ** 2 + 939_549e-10
      * parameter.lengthSCA)
    ...
    let k_torsion = max(
      0.2,
      (-0.0041 * torsion ** 3 - 0.0605 * torsion ** 2 - 0.0354 * torsion
        + 99.997) / 100)
    ...
    let eff =
      shadingSCA * shadingHCE * IAM * edge * k_torsion * wind
  ...
  public mutating func tracking(sun: SolarPosition.Output) {
    ...
    parabolicElevation =
      90
      - (atan(
        tan(sun.zenith.toRadians)
          * cos(((sun.azimuth > 0.0 ? 90.0 : -90.0) - sun.azimuth).toRadians)))
      .toDegrees
    ...
    theta =
      (cos(az - sfaz) / abs(cos(az - sfaz)) * 180 / .pi
        * acos(
          sqrt(
            1
              - pow(
                cos(el - beta) - cos(beta) * cos(el) * (1 - cos(az - sfaz)), 2)
          ))) * (-1)
