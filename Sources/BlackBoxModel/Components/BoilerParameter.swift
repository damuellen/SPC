...
extension Boiler.Parameter: CustomStringConvertible {
  ...
  public var description: String {
    "Description:" * name + "Nominal Outlet Temperature [°]:"
      * nominalTemperatureOut.description + "Minimum Load [%]:"
      * (minLoad * 100).description + "Cold Start-up necessary after [h]:"
      * start.hours.cold.description
      + "Performance needed for Cold Start-up [MWh]:"
      * start.energy.cold.description + "Warm Start-up necessary after [h]:"
      * start.hours.warm.description
      + "Performance needed for Warm Start-up [MWh]:"
      * start.energy.warm.description + "Parasitics at Full Load [MW]:"
      * nominalElectricalParasitics.description
      + "Parasitic Performance Coefficients; "
      + "Parasitics(Load) = Parasitics(100%)*(c0+c1*load)\n" + "c0:"
      * electricalParasitics[0].description + "c1:"
      * electricalParasitics[1].description + "Efficiency; "
...
extension Boiler.Parameter: TextConfigInitializable {
  ...
  init(file: TextConfigFile) throws {
    ...
    self = try .init(
      name: file.name, nominalTemperatureOut: ln(10), minLoad: ln(13),
      nominalElectricalParasitics: ln(34),
      start: .init(
        hours: .init(cold: ln(16), warm: ln(25)),
