...
public enum Boiler: Component {
  ...
  public static func update(_ boiler: inout Boiler.PerformanceData,
    ...
    if case .noOperation = boiler.operationMode { /* || heat >= 0 */
      ...
      noOperation(&boiler, &fuelFlow, availableFuel)
      return 0.0
    }
    ...
    if -thermal / Design.layout.boiler < parameter.minLoad || availableFuel == 0 { // Check if underload
    ...
    if availableFuel < totalFuelNeed { // Check if sufficient fuel avail.
      load(of: &boiler, availableFuel: &availableFuel, fuelFlow: &fuelFlow)
    ...
    if Fuelmode == "predefined" { // predefined fuel consumption in *.pfc-file
      ...
      return fuelFlow * efficiency(at: boiler.load) // net thermal power [MW]
    } else {
      fuelFlow = totalFuelNeed / hourFraction // Fuel flow [MW] in this hour fraction
      return fuelNeed / hourFraction * efficiency(at: boiler.load) // net thermal power [MW]
    }
  }
  ...
  private static func load(of boiler: inout PerformanceData,
    ...
    repeat {
      if Fuelmode == "predefined" { // predefined fuel consumption in *.pfc-file
        ...
        load = availableFuel * efficiency(at: boiler.load) / Design.layout.boiler
