...
extension Storage {
  ...
  static func parasitics(_ status: Storage) -> Power {
    ...
    if parameter.auxConsumptionCurve == false {
      ...
      let pressureLoss =
        parameter.pressureLoss * rohDP / rohMean
        * status.massFlow.share(of: massFlowDP).quotient ** 2
      parasitics =
        pressureLoss * status.massFlow.rate / rohMean
      ...
      if case .noOperation = status.operationMode {
        ...
        let ht = zip(parameter.heatTracingTime, parameter.heatTracingPower)
        for (time, power) in ht {
          if sumMinute > Int(time * 60) { parasitics += power / 1_000 }
        }
        return Power(megaWatt: parasitics)
      }
      ...
      if case .discharge = status.operationMode {
        let load =
          parameter.fixedDischargeLoad.isZero
        ...
        let designDischarge =
          ((((solarField.maxMassFlow - designMassFlow).rate * load)
            ...
            * htf.heatContent(
              ...
              parameter.designTemperature.cold - status.dT_HTFsalt.cold)
            / 1_000) * parameter.heatExchangerEfficiency  // design charging power

        let massFlowDischarging = designDischarge / (hot - cold) * t * 1_000
        let saltFlowRatio = status.salt.active.kg / massFlowDischarging
        parasitics =
          ((1 - lowDc) * designAuxEX * saltFlowRatio ** expn + lowDc
            * designAuxEX)
  ...
  fileprivate func defindedByTonnage() -> Double {
    ...
    return relativeCharge.quotient * Design.layout.storageTonnage
      * (hot - cold) * dT / 3_600
  }
...
extension Storage {
  init(
    operationMode: Storage.OperationMode,
    temperature: (inlet: Temperature, outlet: Temperature),
    temperatureTanks: (cold: Temperature, hot: Temperature)
  ) {
    self.relativeCharge =
      salt.total.kg.isZero ? Ratio(0) : Ratio(salt.hot.kg / salt.total.kg)
    self.temperature = temperature
    self.operationMode = operationMode
    self.temperatureTank = .init(
      cold: temperatureTanks.cold, hot: temperatureTanks.hot)
    ...
    */
    if solarField.pumpParastics.isEmpty {
      ...
      if maxFlow < 900 {
        ...
        SolarField.parameter.pumpParasticsFullLoad =
          (4.7327e-08 * layout ** 4 - 2.0044e-05 * layout ** 3 + 0.0032862
            * layout ** 2 - 0.24086 * layout + 8.2152)
          * (0.7103 * (maxFlow / 597) ** 3 - 0.8236 * (maxFlow / 597) ** 2
            + 1.464 * (maxFlow / 597) - 0.3508)
      } else {
        SolarField.parameter.pumpParasticsFullLoad =
          (5.5e-06 * maxFlow ** 2 - 0.0074 * maxFlow + 4.4)
          * (-1.656e-06 * layout ** 3 + 0.0007981 * layout ** 2 - 0.1322
            * layout + 8.428)
      }
      SolarField.parameter.HTFmass =
        (93300 + 11328 * layout) * (0.63 * (maxFlow / 597) + 0.38)
    }
    ...
    if storage.stepSizeIteration == -99.99 {
      // FIXME
      HeatExchanger.parameter.temperature.h2o.inlet.max =
        storage.startTemperature.hot
      HeatExchanger.parameter.temperature.h2o.inlet.min =
        storage.startTemperature.cold
...
extension Storage: CustomStringConvertible {
  public var description: String {
    "  Mode:".padding(20) + "\(operationMode)".padding(20)
      ...
      + String(format: " T in: %3.1f degC", temperature.inlet.celsius)
      .padding(20)
      + String(format: "T out: %3.1f degC", temperature.outlet.celsius)
      .padding(20) + "," + "  Temperature tanks".padding(28)
      + String(format: " cold: %3.1f degC", temperatureTank.cold.celsius)
      .padding(20)
      + String(format: "  hot: %3.1f degC", temperatureTank.hot.celsius)
      .padding(20) + "," + "  Salt mass".padding(28)
