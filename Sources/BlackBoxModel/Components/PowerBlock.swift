...
struct PowerBlock: Parameterizable, ThermalProcess {
  ...
  enum OperationMode { case scheduledMaintenance }
  ...
  static func parasitics(
    heat: Double, steamTurbine: SteamTurbine, temperature: Temperature
  ) -> Double {
    ...
    case 0...0.5: electricalParasitics += parameter.electricalParasiticsStep[0]
    case 0.5...1: electricalParasitics += parameter.electricalParasiticsStep[1]
    default: break
    }
    ...
    if load > .zero {
      ...
      if parameter.electricalParasiticsACCTamb.coefficients.isEmpty == false {
        ...
        if adjustmentACC > 1 { adjustmentACC = 1 }
  ...
  mutating func temperatureLoss(
    for period: Double, wrt components: ThermalProcess...
  ) {
    if Design.hasGasTurbine {
      ...
      let outletTemperature =
        (massFlow.rate * period * inlet + outlet
          * (HTFmass - massFlowRate * period)) / HTFmass - tlpb
      if outletTemperature > 0 {
        temperature.outlet.kelvin = outletTemperature
      }
    }
  }
}
