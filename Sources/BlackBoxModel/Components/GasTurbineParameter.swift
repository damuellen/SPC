...
extension GasTurbine.Parameter: CustomStringConvertible {
  ...
  public var description: String {
    "Description:" * name
      ...
      + "Efficiency:" * (efficiencyISO * 100).description + "Altitude [m]:"
      * altitude.description + "Efficiency; "
      + "Efficiency(Load) = c0+c1*load+c2*load^2+c3*load^3+c4*load^4)"
      + "\n\(efficiencyFromLoad)" + "Maximum Power as a func of temperature; "
      + "Power(T) = GrossPower*(c0+c1*T+c2*T^2+c3*T^3+c4*T^4)"
      + "\n\(loadMaxFromTemperature)" + "Parasitic as a func of load; "
...
extension GasTurbine.Parameter: TextConfigInitializable {
  ...
  init(file: TextConfigFile) throws {
    ...
    self = try .init(
      name: file.name, powerGross: ln(10), efficiencyISO: ln(13),
      loadMin: ln(16), altitude: ln(19),
