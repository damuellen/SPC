...
extension Recording {
  ...
  func report() -> String {
    ...
    var d: String =
      decorated("PERFORMANCE RUN") + "\nSOLAR FIELD\n" + "    No of Loops:"
      * layout.solarField.description + "    Collector Type:"
      * collector.name.description + "    Aperture [m²]:"
      * aperture.scientificFormatted + "    Massflow [kg/s]:"
      * solarField.maxMassFlow.rate.scientificFormatted
      + "    Elevation [ø]:  " + "\(solarField.elevation)\t Azimut [ø]:  "
      + "\(solarField.azimut)\n" + "\nSTORAGE\n" + "    Capacity [MWH,th]:"
      * (layout.storageHours * steamTurbine.power.max
      / steamTurbine.efficiencyNominal)
      .scientificFormatted + "\nSTEAM TURBINE\n" + "    Gross Output [MW]:"
      * steamTurbine.power.max.description + "    Efficiency [%] :"
      * (steamTurbine.efficiencyNominal * 100).description + "\nGAS TURBINE\n"
      ...
      * "\(layout.gasTurbine * (1 / gasTurbine.efficiencyISO - 1)); \(WasteHeatRecovery.parameter.efficiencyNominal)"
      + "    Efficiency [%]:"
      * WasteHeatRecovery.parameter.efficiencyNominal.description
    ...
    d +=
      ...
      * performance.electric.gross.scientificFormatted
      ...
      + "Parasitic consumption [MWh_el/a]:"
      * performance.parasitics.shared.scientificFormatted
      // Format(YTarS(0).electricalParasitics * (1 - Simulation.parameter.UnSchedMain) * (1 - Simulation.parameter.TransLoss), )"
      + "Net electricty production [MWh_el/a]:"
      * performance.electric.net.scientificFormatted
    ...
    d +=
      ...
      + decorated("Files and Parameter") + "\n" + "Meteodata of a leap year"
      * (Simulation.time.isLeapYear ? "YES" : "NO") + "Location:"
      * String(
        format: "longitude: %G, latitude: %G",
    ...
    d +=
      ...
      * Simulation.parameter.minInsolation.description + "Fuel strategy:"
      * OperationRestriction.fuelStrategy.description
    d += decorated("AVAILABILITIES") + "\n"
    d +=
      Availability.current.description
