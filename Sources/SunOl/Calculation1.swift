      ...
      h[CP + i] = iff(
        or(
          ...
          min(
            ...
            max(
              Double.zero,
              h[CL + i] + min(El_boiler_cap_ud, max(Double.zero, h[CK + i] + max(0, Grid_import_max_ud * Grid_import_yes_no_PB_strategy - max(Double.zero, h[CI + i] - h[L + i])) - h[CM + i] - Overall_harmonious_var_min_cons - Overall_fix_cons))
