extension TunOl {
  func hour0(_ Q_Sol_MW_thLoop: [Double], _ Reference_PV_plant_power_at_inverter_inlet_DC: [Double], _ Reference_PV_MV_power_at_transformer_outlet: [Double]) -> [Double] {
    ...
    for i in 1..<8760 {
      ...
      hour0[O + i] = iff(hour0[J + i] > .zero, hour0[J + i] * CSP_var_aux_nom_perc, CSP_nonsolar_aux_cons) + hour0[M + i] + iff(PB_nom_gross_cap_ud.isZero,0,PB_stby_aux_cons)
      // MAX(0,L6-O6)
    ...
    for i in 1..<8760 { hour0[AM + i] = round(max(.zero, iff(hour0[AH + i] > 0, Grid_import_yes_no_BESS_strategy, Grid_import_yes_no_BESS_strategy_outsideharmop) * Grid_import_max_ud - hour0[AU + i]), 5) }
  ...
  func hour1(hour0: [Double], reserved: Double) -> [Double] {
        ...
        min(
          (Grid_import_max_ud * Grid_import_yes_no_PB_strategy + hour0[AW + i]
            ...
            - iff(
              ...
              PB_stup_aux_cons + PB_warm_start_heat_req * TES_aux_cons_perc, 0) - PB_stby_aux_cons)
    ...
    for i in 1..<8760 {
      let min_net_elec = min(
        hour1[BI + i] + max(.zero, Grid_import_max_ud * Grid_import_yes_no_PB_strategy - hour1[BH + i]) - PB_stby_aux_cons - min(El_boiler_cap_ud, max(.zero, Overall_harmonious_var_heat_min_cons + Overall_heat_fix_cons - hour1[BJ + i]) / El_boiler_eff),
