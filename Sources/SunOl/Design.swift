...
func h_SS(_ t: Float) -> Float { 0.000085999999711 * pow(t, 2) + 1.44300000008555 * t + 4.23842398826006 }

public struct TunOl {

  public static var Grid_import_yes_no_BESS_strategy: Float = 1
  public static var Grid_import_yes_no_PB_strategy: Float = 1

  public static var Q_Sol_MW_thLoop = [Float]()
  public static var Reference_PV_plant_power_at_inverter_inlet_DC = [Float]()
  public static var Reference_PV_MV_power_at_transformer_outlet = [Float]()

  let BESS_chrg_eff: Float = 0.7
  let BESS_chrg_max_cons: Float
  let BESS_chrg_max_ratio: Float = 0.5
  let CCU_cap_min_perc: Float = 0

  let CCU_fix_cons: Float
  let CCU_fix_heat_cons: Float
  let CCU_harmonious_max_perc: Float
  let CCU_harmonious_min_perc: Float

  let CCU_var_heat_nom_cons: Float
  let CCU_var_nom_cons: Float

  let CCU_Ref_CO2_hour_prod: Float = 22
  let CCU_Ref_fix_cons: Float = 0
  let CCU_Ref_heat_fix_cons: Float = 0
  let CCU_Ref_heat_stby_cons: Float = 0
  let CCU_Ref_heat_stup_cons: Float = 0
  let CCU_Ref_stby_cons: Float = 0
  let CCU_Ref_stup_cons: Float = 0
  let CCU_Ref_var_heat_nom_cons: Float = 0
  let CCU_Ref_var_nom_cons: Float = 0

  let CSP_Cold_HTF_T: Float
  let CSP_Hot_HTF_T: Float = 425
  let CSP_night_aux_cons_per_loop: Float = 3.0E-3
  let CSP_nonsolar_aux_cons: Float
  let CSP_var_aux_nom_perc: Float = 0.01
  var CCU_max_perc = [Float](repeating: 0, count: 4)
  var CCU_min_perc = [Float](repeating: 0, count: 4)
  var CO2_max_cons = [Float](repeating: 0, count: 4)
  var CO2_min_cons = [Float](repeating: 0, count: 4)
  var equiv_harmonious_max_perc = [Float](repeating: 0, count: 4)
  var equiv_harmonious_min_perc = [Float](repeating: 0, count: 4)
  var EY_max_perc = [Float](repeating: 0, count: 4)
  var EY_min_perc = [Float](repeating: 0, count: 4)
  var Hydrogen_max_cons = [Float](repeating: 0, count: 4)
  var Hydrogen_min_cons = [Float](repeating: 0, count: 4)
  var MethDist_max_perc = [Float](repeating: 0, count: 4)
  var MethDist_min_perc = [Float](repeating: 0, count: 4)
  var MethSynt_max_perc = [Float](repeating: 0, count: 4)
  var MethSynt_min_perc = [Float](repeating: 0, count: 4)
  var overall_fix_stby_cons = [Float](repeating: 0, count: 4)
  var overall_heat_fix_stby_cons = [Float](repeating: 0, count: 4)
  var overall_heat_stup_cons = [Float](repeating: 0, count: 4)
  var overall_stup_cons = [Float](repeating: 0, count: 4)
  var overall_var_heat_max_cons = [Float](repeating: 0, count: 4)
  var overall_var_heat_min_cons = [Float](repeating: 0, count: 4)
  var overall_var_max_cons = [Float](repeating: 0, count: 4)
  var overall_var_min_cons = [Float](repeating: 0, count: 4)
  var RawMeth_max_cons = [Float](repeating: 0, count: 4)
  var RawMeth_min_cons = [Float](repeating: 0, count: 4)
  let Density_CO2: Float = 945
  let Density_Hydrogen: Float = 5.783
  let El_boiler_eff: Float = 0.99
  var el_Coeff: [Float] = []  // [PB_Eff!$U6, PB_Eff!$T6, PB_Eff!$S6, PB_Eff!$R6, PB_Eff!$Q6]

  let EY_fix_cons: Float
  let EY_Hydrogen_nom_prod: Float

  let EY_harmonious_max_perc: Float
  let EY_harmonious_min_perc: Float
  let EY_harmonious_perc_at_PB_nom: Float
  let EY_heat_fix_cons: Float
  let EY_heat_stby_cons: Float
  let EY_heat_stup_cons: Float
  let EY_Ref_fix_cons: Float = 0

  let EY_var_aux_min_cons: Float
  let EY_var_gross_nom_cons: Float
  let EY_var_heat_nom_cons: Float

  let EY_cap_min_perc: Float = 0.1
  let EY_Ref_heat_fix_cons: Float = 0
  let EY_Ref_heat_stby_cons: Float = 0
  let EY_Ref_heat_stup_cons: Float = 0
  let EY_Ref_stby_cons: Float = 0
  let EY_Ref_stup_cons: Float = 0
  let EY_Ref_var_heat_nom_cons: Float = 0
  let EY_Ref_var_net_nom_cons: Float = 140
  let EY_Ref_var_nom_cons: Float = 0.7

  let fix_aux_el: [Float] = [0.87197586317594244, 0.29314511444598762, -0.49536293286578681, 0.33024195524385674]
  let fix_stby_el: [Float] = [0.8910127968928524, 0.21455823229400395, -0.31671308756057304, 0.21114205837371655]
  let fix_stup_el: [Float] = [0.89405519494415775, 0.19885240173966706, -0.27872279005147399, 0.18581519336764935]
  let Heater_eff: Float = 0.96
  let Heater_outlet_T: Float = 565
  let HL_Coeff: [Float]
  let Inv_eff_Ref_approx_handover: Float  // Inv_Eff!C22
  let LL_Coeff: [Float]
  let MethDist_cap_min_perc: Float = 0.5

  let MethDist_fix_cons: Float
  let MethDist_HydrogenO_min_prod: Float

  let MethDist_harmonious_max_perc: Float
  let MethDist_harmonious_min_perc: Float

  let MethDist_harmonious_perc_at_PB_nom: Float
  let MethDist_RawMeth_nom_cons: Float

  let MethDist_Ref_fix_cons: Float = 0
  let MethDist_Ref_heat_fix_cons: Float = 0
  let MethDist_Ref_heat_stby_cons: Float = 0
  let MethDist_Ref_heat_stup_cons: Float = 0
  let MethDist_Ref_meth_annual_prod: Float
  let MethDist_Ref_meth_hour_prod: Float
  let MethDist_Ref_water_hour_prod: Float
  let MethDist_Ref_stby_cons: Float = 0
  let MethDist_Ref_stup_cons: Float = 0
  let MethDist_Ref_var_heat_nom_cons: Float
  let MethDist_Ref_var_nom_cons: Float = 0.01
  let MethDist_Ref_water_annual_prod: Float

  let MethSynt_annual_outage_days: Float = 0
  let MethSynt_cap_min_perc: Float = 0.1
  let MethSynt_CO2_min_cons: Float
  let MethSynt_CO2_nom_cons: Float
  let MethSynt_fix_cons: Float
  let MethSynt_Hydrogen_nom_cons: Float
  let MethSynt_harmonious_max_perc: Float
  let MethSynt_harmonious_min_perc: Float
  let MethSynt_heat_fix_prod: Float

  let MethSynt_Ref_Hydrogen_hour_cons: Float
  let MethSynt_Ref_rawmeth_hour_prod: Float
  let MethSynt_var_heat_nom_prod: Float
  let MethSynt_var_nom_cons: Float
  let MethSynt_Ref_CO2_annual_cons: Float

  let MethSynt_Ref_fix_cons: Float = 5.25
  let MethSynt_Ref_Hydrogen_annual_cons: Float

  let MethSynt_Ref_CO2_hour_cons: Float
  let MethSynt_Ref_heat_fix_prod: Float = 0
  let MethSynt_Ref_heat_stby_cons: Float = 0
  let MethSynt_Ref_heat_stup_cons: Float = 0

  let MethSynt_Ref_stby_cons: Float = 0
  let MethSynt_Ref_stup_cons: Float = 0
  let MethSynt_Ref_var_heat_nom_prod: Float = 7
  let MethSynt_Ref_var_nom_cons: Float = 0
  let MethSynt_stup_duration: Float = 15

  let Overall_harmonious_max_perc: Float = 1
  let Overall_fix_cons: Float
  let Overall_harmonious_min_perc: Float
  let Overall_harmonious_var_heat_cons_at_PB_nom: Float
  let Overall_harmonious_var_heat_max_cons: Float
  let Overall_harmonious_var_heat_min_cons: Float
  let Overall_harmonious_var_max_cons: Float
  let Overall_harmonious_var_min_cons: Float
  let Overall_heat_fix_cons: Float

  let PB_cold_start_duration: Float = 48
  let PB_cold_start_energyperc: Float = 2
  var PB_cold_start_heat_req: Float = 0
  let PB_fix_aux_el: Float
  let PB_fix_aux_elec_cons_perc_of_ref: Float = 6.4340704330890603E-3
  var PB_gross_min_cap: Float = 0
  var PB_gross_min_eff: Float = 0
  var PB_heat_min_input: Float = 0
  let PB_hot_start_energyperc: Float = 0.05
  var PB_hot_start_heat_req: Float = 0

  var PB_n_g_var_aux_el_Coeff: [Float] = [0]  // PB_Eff!O49 PB_Eff!N49 PB_Eff!M49 PB_Eff!L49

  let PB_net_min_cap: Float
  var PB_nom_gross_eff: Float = 0.4  // PB_Eff!I16
  var PB_nom_heat_cons: Float = 0
  let PB_nom_net_cap: Float
  let PB_nom_var_aux_cons: Float
  let PB_nom_var_aux_cons_perc_gross: Float = 3.3931144552042103E-2
  var PB_nom_var_aux_cons_perc_net: Float = 0  // PB_Eff!I18
  let PB_op_hours_min_nr: Float = 5
  var PB_Ratio_Heat_input_vs_output: Float = 0  // PB_Eff!I14

  let PB_Ref_25p_gross_cap: Float = 25
  let PB_Ref_25p_gross_cap_max_aux_heat: Float = 25

  let PB_Ref_25p_gross_eff_excl_aux_heat_cons: Float = 0  // PB_Eff!E22
  let PB_Ref_25p_gross_eff_excl_max_aux_heat_cons: Float = 0  // PB_Eff!E24

  let PB_Ref_25p_heat_cons: Float = 85.512
  // let PB_Ref_25p_heat_cons_excl_aux_heat: Float = 0  // PB_Eff!E21
  // let PB_Ref_25p_heat_cons_excl_max_aux_heat: Float = 0  // PB_Eff!E23

  let PB_Ref_25p_heat_cons_max_aux_heat: Float = 112.349

  let PB_Ref_30p_gross_cap: Float = 63

  // let PB_Ref_30p_gross_eff_excl_aux_heat_cons: Float = 0  // PB_Eff!E20

  let PB_Ref_30p_heat_cons: Float = 168.78700000000001
  // let PB_Ref_30p_heat_cons_excl_aux_heat: Float = 0  // PB_Eff!E19

  let PB_Ref_nom_gross_cap: Float = 200

  // let PB_Ref_nom_gross_eff_excl_aux_heat_cons: Float = 0  // PB_Eff!E18

  let PB_Ref_nom_heat_cons: Float = 463.48500000000001
  // let PB_Ref_nom_heat_cons_excl_aux_heat: Float = 0  // PB_Eff!E17
  let PB_stby_aux_cons: Float
  let PB_stby_fix_aux_cons_perc: Float = 5.2640704330890603E-3
  let PB_stby_var_aux_cons_perc: Float = 4.6250983512691503E-3
  let PB_stup_aux_cons: Float
  let PB_stup_fix_aux_elec_cons_perc: Float = 5.9815704330890597E-3
  let PB_stup_var_aux_elec_cons_perc: Float = 1.2789153998036399E-2
  // let PB_var_aux_cons = [0.29437013201591916, 0.10280513176871063, -6.5249624421765337E-2, 0.67514642417652304]
  let PB_g2n_var_aux_el_Coeff: [Float] = [0.29437013201591916, 0.10280513176871063, -6.5249624421765337E-2, 0.67514642417652304]  // PB_var_aux_cons_C0
  var PB_var_heat_max_cons: Float = 0  // PB_Eff!I17

  let PB_warm_start_duration: Int = 6
  let PB_warm_start_energyperc: Float = 0.4
  var PB_warm_start_heat_req: Float = 0
  let PV_Ref_AC_cap: Float = 510  //max(Calculation!G5,G8764)
  let PV_Ref_DC_cap: Float = 683.4
  let Ratio_CSP_vs_Heater: Float
  let SF_heat_exch_approach_temp: Float = 7
  let TES_aux_cons_perc: Float = 0.01
  let TES_cold_tank_T: Float = 304.55
  let TES_dead_mass_ratio: Float = 0.1
  var TES_salt_mass: Float = 0
  var TES_thermal_cap: Float = 0
  var TES_full_load_hours: Float = 0

  var th_Coeff: [Float] = [0]  // PB_Eff!$U3 PB_Eff!$T3 PB_Eff!$S3 PB_Eff!$R3 PB_Eff!$Q3

  var CSP_loop_nr_ud: Float
  var TES_thermal_cap_ud: Float
  var PB_nom_gross_cap_ud: Float
  var PV_AC_cap_ud: Float
  var PV_DC_cap_ud: Float
  var EY_var_net_nom_cons_ud: Float
  var Hydrogen_storage_cap_ud: Float
  var Heater_cap_ud: Float
  var CCU_CO2_nom_prod_ud: Float
  var CO2_storage_cap_ud: Float
  var MethSynt_RawMeth_nom_prod_ud: Float
  var RawMeth_storage_cap_ud: Float
  var MethDist_Meth_nom_prod_ud: Float
  var El_boiler_cap_ud: Float
  var BESS_cap_ud: Float
  var Grid_export_max_ud: Float
  var Grid_import_max_ud: Float
  var Grid_export_yes_no_PB_strategy: Float = 1
  var Grid_import_yes_no_BESS_strategy: Float = 0
  var Grid_import_yes_no_PB_strategy: Float = 0
  var Grid_import_yes_no_PB_strategy_outsideharmop: Float = 0
  var Grid_import_yes_no_BESS_strategy_outsideharmop: Float = 0
  var Grid_export_yes_no_BESS_strategy: Float = 1
  var Grid_export_yes_no_PB_strategy_outsideharmop: Float = 1
  var Grid_export_yes_no_BESS_strategy_outsideharmop: Float = 1
  var EDG_elec_cost_factor: Float = 1
  ...
  init?(_ parameter: [Float]) {
    ...
    let eff1 = chunks.map { bin in bin.reduce(0.0) { $0 + $1.1 } / Float(bin.count) }
    let s = Array(stride(from: Float(0.01), through: Float(1), by: Float(0.01)))
    let eff2 = zip(s, eff1).map { ac * $0.0 / $0.1 / dc }

    self.LL_Coeff = fit(x: Array(eff2[..<20]), y: Array(eff1[..<20]), order: 7)!
    self.HL_Coeff = fit(x: Array(eff2[15...].dropLast(2)), y: Array(eff1[15...].dropLast(2)), order: 3)!
    ...
    let EY_Ref_Hydrogen_hour_nom_prod: Float = 2.8
    ...
    if PB_nom_gross_cap_ud.isZero {
      ...
      let PB_Ref_25p_aux_heat_prod: Float = (7.194 * 2775.4 - 5.414 * 167.6 - 1.78 * 649.6) / 1000
      let PB_Ref_30p_aux_heat_prod: Float = (10.778 * 2775.4 - 8.278 * 167.6 - 2.5 * 649.6) / 1000
      let PB_Ref_nom_aux_heat_prod: Float = (23.333 * 2775.4 - 15.167 * 167.6 - 8.167 * 649.6) / 1000

      let gross_cap: [Float] = [  // A29-A33
      ...
      let load_perc: [Float] = gross_cap.map { $0 / PB_Ref_nom_gross_cap }  // B
      let PB_Ref_25p_max_aux_heat_prod: Float = (23.333 * 2775.4 - 15.167 * 167.6 - 8.167 * 649.6) / 1000
      let heat_input = load_perc.map { $0 * PB_nom_gross_cap_ud }  // D
      let factor = seek(goal: Float(0)) { (PB_Ref_25p_heat_cons - PB_Ref_25p_aux_heat_prod * $0) - (PB_Ref_25p_heat_cons_max_aux_heat - PB_Ref_25p_max_aux_heat_prod * $0) }  // 0.68792724609375

      let no_extraction: [Float] = [  // B
      ...
      let steam_extraction: [Float] = net_electrical_output.map { output -> Float in return iff(PB_nom_gross_cap_ud.isZero, .zero, (Overall_harmonious_var_heat_cons_at_PB_nom + Overall_heat_fix_cons) / PB_nom_net_cap * output) }  // F

      self.th_Coeff = fit(x: thermal_load_perc, y: eff_factor, order: 4)!
      self.el_Coeff = fit(x: load_perc, y: eff_factor, order: 4)!
      ...
      self.PB_n_g_var_aux_el_Coeff = fit(x: net_el_output_factor, y: auxiliary_consumption_factor, order: 4)!
...
func seek(
  goal: Float, _ range: ClosedRange<Float> = 0...1, tolerance: Float = 0.0001,
  maxIterations: Int = 100, _ f: (Float) -> Float
) -> Float {
...
func fit<S: Collection>(
    ...
    var P = [Float](repeating: 0.0, count: ((order + 1) * 2) + 1)
    var A = [Float](repeating: 0.0, count: (order + 1) * 2 * (order + 1))
    var coefficients = [Float](repeating: 0.0, count: order + 1)
