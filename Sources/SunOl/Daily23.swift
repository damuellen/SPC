extension TunOl {
  func d23(_ d23: inout [Double], case j: Int, day0: [Double], d21: [Double], d22: [Double]) {
    ...
    let AE = 9490
    let (EA, EB, EC, EE, EF, EG, EH, EI, EJ, EK, EL, EM, EN, EO, EP, EQ, ER, ES, ET, EV, EW, EY, EZ) = (
      3285, 3650, 4015, 4745, 5110, 5475, 5840, 6205, 6570, 6935, 7300, 7665, 8030, 8395, 8760, 9125, 9490, 9855, 10220, 10950, 11315, 12045, 12410
    )
    let (DR, DS, DT, DU, DV, DW, DX) = (0, 365, 730, 1095, 1460, 1825, 2190)

    /// Surplus harm op period el after min day harm op and min night op prep
    let FC = 0
    // =EB6+EH6-O6-MIN(EK6,MAX(0,EA6+E6+MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN(EP6,MAX(0,Q6-EE6)/El_boiler_eff)
    for i in 0..<365 {
      d23[FC + i] =
        d22[EB + i] + d22[EH + i] - d21[O + i] - min(d22[EK + i], max(0.0, d22[EA + i] + d21[E + i] + min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff)
        - min(d22[EP + i], max(0.0, d21[Q + i] - d22[EE + i]) / El_boiler_eff)
    }

    /// Surplus harm op period el after min day harm op and max night op prep
    let FD = 365
    // =EB6+EH6-P6-MIN(EK6,MAX(0,EA6+F6+MIN(ER6,MAX(0,H6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN(EP6,MAX(0,R6-EE6)/El_boiler_eff)
    for i in 0..<365 {
      d23[FD + i] =
        d22[EB + i] + d22[EH + i] - d21[P + i] - min(d22[EK + i], max(0.0, d22[EA + i] + d21[F + i] + min(d22[ER + i], max(0.0, d21[H + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff)
        - min(d22[EP + i], max(0.0, d21[R + i] - d22[EE + i]) / El_boiler_eff)
    }

    /// Surplus harm op period el after max day harm op and min night op prep
    let FE = 730
    // =EC6+EI6-O6-MIN(EL6,MAX(0,EA6+E6+MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN(EQ6,MAX(0,Q6-EF6)/El_boiler_eff)
    for i in 0..<365 {
      d23[FE + i] =
        d22[EC + i] + d22[EI + i] - d21[O + i] - min(d22[EL + i], max(0.0, d22[EA + i] + d21[E + i] + min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff)
        - min(d22[EQ + i], max(0.0, d21[Q + i] - d22[EF + i]) / El_boiler_eff)
    }
    ...
    let FF = 1095
    // =(EK6+EM6)*BESS_chrg_eff+EJ6-EA6-E6-MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)
    for i in 0..<365 { d23[FF + i] = (d22[EK + i] + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[E + i] - min(d22[ER + i], max(.zero, d21[G + i] - d22[EG + i]) / El_boiler_eff) }
    ...
    let FG = 1460
    // =(EK6+EM6)*BESS_chrg_eff+EJ6-EA6-F6-MIN(ER6,MAX(0,H6-EG6)/El_boiler_eff)
    for i in 0..<365 { d23[FG + i] = (d22[EK + i] + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[F + i] - min(d22[ER + i], max(0.0, d21[H + i] - d22[EG + i]) / El_boiler_eff) }
    ...
    let FH = 1825
    // =(EL6+EM6)*BESS_chrg_eff+EJ6-EA6-E6-MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)
    for i in 0..<365 { d23[FH + i] = (d22[EL + i] + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[E + i] - min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff) }
    ...
    let FI = 2190
    // =EE6+MIN(EP6,MAX(0,EB6+EH6-MIN(EK6,MAX(0,EA6+E6+MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-O6))*El_boiler_eff-Q6
    for i in 0..<365 {
      d23[FI + i] =
        d22[EE + i] + min(
          d22[EP + i],
          max(0.0, d22[EB + i] + d22[EH + i] - min(d22[EK + i], max(0.0, d22[EA + i] + d21[E + i] + min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff) - d21[O + i]))
        * El_boiler_eff - d21[Q + i]
    }
    ...
    let FJ = 2555
    // =EE6+MIN(EP6,MAX(0,EB6+EH6-MIN(EK6,MAX(0,EA6+F6+MIN(ER6,MAX(0,H6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-P6))*El_boiler_eff-R6
    for i in 0..<365 {
      d23[FJ + i] =
        d22[EE + i] + min(
          d22[EP + i],
          max(0.0, d22[EB + i] + d22[EH + i] - min(d22[EK + i], max(0.0, d22[EA + i] + d21[F + i] + min(d22[ER + i], max(0.0, d21[H + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff) - d21[P + i]))
        * El_boiler_eff - d21[R + i]
    }
    ...
    let FK = 2920
    // =EF6+MIN(EQ6,MAX(0,EC6+EI6-MIN(EL6,MAX(0,EA6+E6+MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-O6))*El_boiler_eff-Q6
    for i in 0..<365 {
      d23[FK + i] =
        d22[EF + i] + min(
          d22[EQ + i],
          max(0.0, d22[EC + i] + d22[EI + i] - min(d22[EL + i], max(0.0, d22[EA + i] + d21[E + i] + min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff) - d21[O + i]))
        * El_boiler_eff - d21[Q + i]
    }
    ...
    for i in 0..<365 {
      ...
      d23[FL + i] = d22[EG + i] + min(d22[ER + i], max(0.0, (d22[EK + i] + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[E + i])) * El_boiler_eff - d21[G + i]
    ...
    for i in 0..<365 {
      // =IF(El_boiler_cap_ud=0,0,$ER6-MAX(0,$G6-$EG6)/El_boiler_eff)
      if El_boiler_cap_ud.isZero { d23[FT + i] = Double.zero } else { d23[FT + i] = d22[ER + i] - max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff }
      // ES6-S6
    ...
    for i in 0..<365 {
      d23[GE + i] = iff(
        or(d21[AE + i].isZero, d23[FC + i] < 0.0, d23[FF + i] < 0.0, d23[FI + i] < 0.0, d23[FL + i] < 0.0, d23[FO + i] < 0.0, d23[FR + i] < 0.0, d23[FU + i] < 0.0, d23[FX + i] < 0.0, d23[GA + i] < 0), 0.0,
        min(
          1, ifFinite(d23[FC + i] / (d23[FC + i] - d23[FE + i]), 1), ifFinite(d23[FF + i] / max(0.0, d23[FF + i] - d23[FH + i]), 1), ifFinite(d23[FI + i] / max(0.0, d23[FI + i] - d23[FK + i]), 1),
          ifFinite(d23[FL + i] / max(0.0, d23[FL + i] - d23[FN + i]), 1), ifFinite(d23[FO + i] / max(0.0, d23[FO + i] - d23[FQ + i]), 1), ifFinite(d23[FR + i] / max(0.0, d23[FR + i] - d23[FT + i]), 1),
          ifFinite(d23[FU + i] / max(0.0, d23[FU + i] - d23[FW + i]), 1), ifFinite(d23[FX + i] / max(0.0, d23[FX + i] - d23[FZ + i]), 1), ifFinite(d23[GA + i] / max(0.0, d23[GA + i] - d23[GC + i]), 1)) * Overall_harmonious_range
    ...
    let GF = 10220
    // =IF(GE6=0,0,ROUND((EB6+(EC6-EB6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))+(EH6+(EI6-EH6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))-O6-MIN((EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)),MAX(0,EA6+E6+MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN((EP6+(EQ6-EP6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)),MAX(0,Q6-(EE6+(EF6-EE6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)))/El_boiler_eff),5))
    for i in 0..<365 {
      d23[GF + i] = iff(
        d23[GE + i].isZero, Double.zero,
        round(
          (d22[EB + i] + (d22[EC + i] - d22[EB + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc))
            + (d22[EH + i] + (d22[EI + i] - d22[EH + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)) - d21[O + i]
            - min(
              (d22[EK + i] + (d22[EL + i] - d22[EK + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)),
            ...
            - min(
              (d22[EP + i] + (d22[EQ + i] - d22[EP + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)),
              max(0.0, d21[Q + i] - (d22[EE + i] + (d22[EF + i] - d22[EE + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc))) / El_boiler_eff), 5))
    }

    /// Surplus harm op period el after opt day harm op and max night op prep
    let GG = 10585
    // =IF(GE6=0,0,ROUND((EB6+(EC6-EB6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))+(EH6+(EI6-EH6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))-P6-MIN((EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)),MAX(0,EA6+F6+MIN(ER6,MAX(0,H6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN((EP6+(EQ6-EP6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)),MAX(0,R6-(EE6+(EF6-EE6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)))/El_boiler_eff),5))
    for i in 0..<365 {
      d23[GG + i] = iff(
        d23[GE + i].isZero, Double.zero,
        round(
          (d22[EB + i] + (d22[EC + i] - d22[EB + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc))
            + (d22[EH + i] + (d22[EI + i] - d22[EH + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)) - d21[P + i]
            - min(
              (d22[EK + i] + (d22[EL + i] - d22[EK + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)),
              max(0.0, d22[EA + i] + d21[F + i] + min(d22[ER + i], max(0.0, d21[H + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff)
            - min(
              (d22[EP + i] + (d22[EQ + i] - d22[EP + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)),
              max(0.0, d21[R + i] - (d22[EE + i] + (d22[EF + i] - d22[EE + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc))) / El_boiler_eff), 5))
    }
    ...
    let GH = 10950
    // =IF(GE6=0,0,ROUND(((EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))+EM6)*BESS_chrg_eff+EJ6-EA6-E6-MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff),5))
    for i in 0..<365 {
      d23[GH + i] = iff(
        d23[GE + i].isZero, 0.0,
        round(((d22[EK + i] + (d22[EL + i] - d22[EK + i]) * d23[ddGE + i]) + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[E + i] - min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff), 5))
    }
    ...
    let GI = 11315
    // =IF(GE6=0,0,ROUND(((EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))+EM6)*BESS_chrg_eff+EJ6-EA6-F6-MIN(ER6,MAX(0,H6-EG6)/El_boiler_eff),5))
    for i in 0..<365 {
      d23[GI + i] = iff(
        d23[GE + i].isZero, 0.0,
        round(((d22[EK + i] + (d22[EL + i] - d22[EK + i]) * d23[ddGE + i]) + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[F + i] - min(d22[ER + i], max(0.0, d21[H + i] - d22[EG + i]) / El_boiler_eff), 5))
    }
    ...
    let GJ = 11680
    // =IF(GE6=0,0,ROUND(EE6+(EF6-EE6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)+MIN(EP6+(EQ6-EP6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc),MAX(0,EB6+(EC6-EB6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)+EH6+(EI6-EH6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)-MIN((EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)),MAX(0,EA6+E6+MIN(ER6,MAX(0,G6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-O6))*El_boiler_eff-Q6,5))
    for i in 0..<365 {
      d23[GJ + i] = iff(
        d23[GE + i].isZero, Double.zero,
        round(
          d22[EE + i] + (d22[EF + i] - d22[EE + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc) + min(
            d22[EP + i] + (d22[EQ + i] - d22[EP + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc),
            max(
              0.0,
              d22[EB + i] + (d22[EC + i] - d22[EB + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc) + d22[EH + i] + (d22[EI + i] - d22[EH + i])
                / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)
                - min(
                  (d22[EK + i] + (d22[EL + i] - d22[EK + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)),
                  max(0.0, d22[EA + i] + d21[E + i] + min(d22[ER + i], max(0.0, d21[G + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff) - d21[O + i])) * El_boiler_eff - d21[Q + i], 5))
    }
    ...
    let GK = 12045
    // =IF(GE6=0,0,ROUND(EE6+(EF6-EE6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)+MIN(EP6+(EQ6-EP6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc),MAX(0,EB6+(EC6-EB6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)+EH6+(EI6-EH6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)-MIN((EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)),MAX(0,EA6+F6+MIN(ER6,MAX(0,H6-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-P6))*El_boiler_eff-R6,5))
    for i in 0..<365 {
      d23[GK + i] = iff(
        d23[GE + i].isZero, Double.zero,
        round(
          d22[EE + i] + (d22[EF + i] - d22[EE + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc) + min(
            ...
            max(
              ...
              d22[EB + i] + (d22[EC + i] - d22[EB + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc) + d22[EH + i] + (d22[EI + i] - d22[EH + i])
                ...
                - min(
                  (d22[EK + i] + (d22[EL + i] - d22[EK + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)),
                  max(0.0, d22[EA + i] + d21[F + i] + min(d22[ER + i], max(0.0, d21[H + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff) / BESS_chrg_eff) - d21[P + i])) * El_boiler_eff - d21[R + i], 5))
    }
    ...
    for i in 0..<365 {
      d23[GL + i] = iff(
        ...
        round(
          d22[EG + i] + min(
            d22[ER + i],
            max(0.0, (d22[EK + i] + (d22[EL + i] - d22[EK + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc) + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[E + i]))
            * El_boiler_eff - d21[G + i], 5))
    }
    ...
    for i in 0..<365 {
      d23[GM + i] = iff(
        ...
        round(
          d22[EG + i] + min(
            d22[ER + i],
            max(0.0, (d22[EK + i] + (d22[EL + i] - d22[EK + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc) + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - d21[F + i]))
            * El_boiler_eff - d21[H + i], 5))
    }
    ...
    for i in 0..<365 {
      d23[GN + i] = iff(
        or(d23[GE + i].isZero, El_boiler_cap_ud.isZero), 0.0, round((d22[EP + i] + (d22[EQ + i] - d22[EP + i]) * d23[ddGE + i]) - max(0.0, d21[Q + i] - (d22[EE + i] + (d22[EF + i] - d22[EE + i]) * d23[ddGE + i])) / El_boiler_eff, 5))
    }
      ...
      d23[GO + i] = iff(
        ...
        round(
          (d22[EP + i] + (d22[EQ + i] - d22[EP + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)) - max(
            0.0, d21[R + i] - (d22[EE + i] + (d22[EF + i] - d22[EE + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc))) / El_boiler_eff, 5))
    }
    ...
    for i in 0..<365 { d23[GR + i] = iff(d23[GE + i].isZero, 0.0, round((d22[ES + i] + (d22[ET + i] - d22[ES + i]) * d23[ddGE + i]) - d21[S + i], 5)) }
    ...
    for i in 0..<365 { d23[GS + i] = iff(d23[GE + i].isZero, 0.0, round((d22[ES + i] + (d22[ET + i] - d22[ES + i]) * d23[ddGE + i]) - d21[T + i], 5)) }
    ...
    for i in 0..<365 { d23[GT + i] = iff(d23[GE + i].isZero, 0.0, round((d22[EV + i] + (d22[EW + i] - d22[EV + i]) * d23[ddGE + i]) - d21[U + i], 5)) }
    ...
    for i in 0..<365 { d23[GU + i] = iff(d23[GE + i].isZero, 0.0, round((d22[EV + i] + (d22[EW + i] - d22[EV + i]) * d23[ddGE + i]) - d21[V + i], 5)) }
    ...
    for i in 0..<365 { d23[GV + i] = iff(d23[GE + i].isZero, 0.0, round((d22[EY + i] + (d22[EZ + i] - d22[EY + i]) * d23[ddGE + i]) - d21[W + i], 5)) }
    ...
    for i in 0..<365 { d23[GW + i] = iff(d23[GE + i].isZero, 0.0, round((d22[EY + i] + (d22[EZ + i] - d22[EY + i]) * d23[ddGE + i]) - d21[X + i], 5)) }
    ...
    for i in 0..<365 {
      d23[GX + i] = iff(
        or(d23[GE + i].isZero, d23[GF + i] < 0.0, d23[GH + i] < 0.0, d23[GJ + i] < 0.0, d23[GL + i] < 0.0, d23[GN + i] < 0.0, d23[GP + i] < 0.0, d23[GR + i] < 0.0, d23[GT + i] < 0.0, d23[GV + i] < 0), 0.0,
        min(
          1, ifFinite(d23[GF + i] / max(0.0, d23[GF + i] - d23[GG + i]), 1), ifFinite(d23[GH + i] / max(0.0, d23[GH + i] - d23[GI + i]), 1), ifFinite(d23[GJ + i] / max(0.0, d23[GJ + i] - d23[GK + i]), 1),
          ifFinite(d23[GL + i] / max(0.0, d23[GL + i] - d23[GM + i]), 1), ifFinite(d23[GN + i] / max(0.0, d23[GN + i] - d23[GO + i]), 1), ifFinite(d23[GP + i] / max(0.0, d23[GP + i] - d23[GQ + i]), 1),
          ifFinite(d23[GR + i] / max(0.0, d23[GR + i] - d23[GS + i]), 1), ifFinite(d23[GT + i] / max(0.0, d23[GT + i] - d23[GU + i]), 1), ifFinite(d23[GV + i] / max(0.0, d23[GV + i] - d23[GW + i]), 1))
          * (d21[AE + i] - equiv_harmonious_min_perc[j]) + equiv_harmonious_min_perc[j])
    }
    ...
    for i in 0..<365 {
      d23[GZ + i] = iff(
        or(d21[AE + i].isZero, d23[FC + i] < 0.0, d23[FF + i] < 0.0, d23[FI + i] < 0.0, d23[FL + i] < 0.0, d23[FO + i] < 0.0, d23[FR + i] < 0.0, d23[FU + i] < 0.0, d23[FX + i] < 0.0, d23[GA + i] < 0), 0.0,
        min(
          1, ifFinite(d23[FC + i] / max(0.0, d23[FC + i] - d23[FD + i]), 1), ifFinite(d23[FF + i] / max(0.0, d23[FF + i] - d23[FG + i]), 1), ifFinite(d23[FI + i] / max(0.0, d23[FI + i] - d23[FJ + i]), 1),
          ifFinite(d23[FL + i] / max(0.0, d23[FL + i] - d23[FM + i]), 1), ifFinite(d23[FO + i] / max(0.0, d23[FO + i] - d23[FP + i]), 1), ifFinite(d23[FR + i] / max(0.0, d23[FR + i] - d23[FS + i]), 1),
          ifFinite(d23[FU + i] / max(0.0, d23[FU + i] - d23[FV + i]), 1), ifFinite(d23[FX + i] / max(0.0, d23[FX + i] - d23[FY + i]), 1), ifFinite(d23[GA + i] / max(0.0, d23[GA + i] - d23[GB + i]), 1))
          * (d21[AE + i] - equiv_harmonious_min_perc[j]) + equiv_harmonious_min_perc[j])
    }
    ...
    for i in 0..<365 {
      ...
      // =IF(GZ6=0,0,ROUND(EB6+EH6-(O6+(P6-O6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))-MIN(EK6,MAX(0,EA6+(E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))+MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN(EP6,MAX(0,Q6+(R6-Q6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EE6)/El_boiler_eff),5))
      d23[HA + i] = round(
        d22[EB + i] + d22[EH + i] - (d21[O + i] + (d21[P + i] - d21[O + i]) * d23[ddGZ + i])
          - min(
            d22[EK + i],
            max(
              0.0,
              d22[EA + i] + (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i]) + min(d22[ER + i], max(0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff)
              / BESS_chrg_eff) - min(d22[EP + i], max(0.0, d21[Q + i] + (d21[R + i] - d21[Q + i]) * d23[ddGZ + i] - d22[EE + i]) / El_boiler_eff), 5)

      // =IF(GZ6=0,0,ROUND(EC6+EI6-(O6+(P6-O6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))-MIN(EL6,MAX(0,EA6+(E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))+MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-MIN(EQ6,MAX(0,Q6+(R6-Q6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EF6)/El_boiler_eff),5))
      d23[HB + i] = round(
        d22[EC + i] + d22[EI + i] - (d21[O + i] + (d21[P + i] - d21[O + i]) * d23[ddGZ + i])
          - min(
            d22[EL + i],
            max(
              0.0,
              d22[EA + i] + (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i]) + min(d22[ER + i], max(0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff)
              / BESS_chrg_eff) - min(d22[EQ + i], max(0.0, d21[Q + i] + (d21[R + i] - d21[Q + i]) * d23[ddGZ + i] - d22[EF + i]) / El_boiler_eff), 5)

      // =IF(GZ6=0,0,ROUND((EK6+EM6)*BESS_chrg_eff+EJ6-EA6-(E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))-MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff),5))

      d23[HC + i] = round(
        (d22[EK + i] + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i])
          - min(d22[ER + i], max(0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i] - d22[EG + i]) / El_boiler_eff), 5)
      // =IF(GZ6=0,0,ROUND((EL6+EM6)*BESS_chrg_eff+EJ6-EA6-(E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))-MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff),5))

      d23[HD + i] = round(
        (d22[EL + i] + d22[EM + i]) * BESS_chrg_eff + d22[EJ + i] - d22[EA + i] - (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i])
          - min(d22[ER + i], max(0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i] - d22[EG + i]) / El_boiler_eff), 5)
      // =IF(GZ6=0,0,ROUND(EE6+MIN(EP6,MAX(0,EB6+EH6-MIN(EK6,MAX(0,EA6+E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)+MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-(O6+(P6-O6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))))*El_boiler_eff-(Q6+(R6-Q6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)),5))
          ...
          max(
            ...
            d22[EB + i] + d22[EH + i]
              - min(
                ...
                max(
                  0.0,
                  d22[EA + i] + (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i]) + min(d22[ER + i], max(0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i] - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i]
                    * BESS_chrg_eff) / BESS_chrg_eff) - (d21[O + i] + (d21[P + i] - d21[O + i]) * d23[ddGZ + i]))) * El_boiler_eff - (d21[Q + i] + (d21[R + i] - d21[Q + i]) * d23[ddGZ + i]), 5)

      // =IF(GZ6=0,0,ROUND(EF6+MIN(EQ6,MAX(0,EC6+EI6-MIN(EL6,MAX(0,EA6+E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)+MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff)-(O6+(P6-O6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc))))*El_boiler_eff-(Q6+(R6-Q6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)),5))
      d23[HF + i] = round(
        d22[EF + i] + min(
          d22[EQ + i],
          max(
            ...
            d22[EC + i] + d22[EI + i] - max(
              0.0, d22[EA + i] + (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i]) + max(0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i] - d22[EG + i]) / El_boiler_eff - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff)
              / BESS_chrg_eff - (d21[O + i] + (d21[P + i] - d21[O + i]) * d23[ddGZ + i]))) * El_boiler_eff - (d21[Q + i] + (d21[R + i] - d21[Q + i]) * d23[ddGZ + i]), 5)
    ...
    for i in 0..<365 {
      if 24255 == i + HS {}
      d23[HS + i] = iff(
        or(d23[GZ + i].isZero, d23[HA + i] < 0.0, d23[HC + i] < 0.0, d23[HE + i] < 0.0, d23[HG + i] < 0.0, d23[HI + i] < 0.0, d23[HK + i] < 0.0, d23[HM + i] < 0.0, d23[HO + i] < 0.0, d23[HQ + i] < 0), 0.0,
        min(
          1, ifFinite(d23[HA + i] / max(0.0, d23[HA + i] - d23[HB + i]), 1), ifFinite(d23[HC + i] / max(0.0, d23[HC + i] - d23[HD + i]), 1), ifFinite(d23[HE + i] / max(0.0, d23[HE + i] - d23[HF + i]), 1),
          ifFinite(d23[HG + i] / max(0.0, d23[HG + i] - d23[HH + i]), 1), ifFinite(d23[HI + i] / max(0.0, d23[HI + i] - d23[HJ + i]), 1), ifFinite(d23[HK + i] / max(0.0, d23[HK + i] - d23[HL + i]), 1),
          ifFinite(d23[HM + i] / max(0.0, d23[HM + i] - d23[HN + i]), 1), ifFinite(d23[HO + i] / max(0.0, d23[HO + i] - d23[HP + i]), 1), ifFinite(d23[HQ + i] / max(0.0, d23[HQ + i] - d23[HR + i]), 1)) * Overall_harmonious_range
          + Overall_harmonious_min_perc)
    }
    ...
    for i in 0..<365 {
      ...
      d23[ID + i] = iff(d22[DT + i].isZero, 0.0, iff(d23[GE + i].isZero, d22[DU + i], d22[DT + i] + (d22[DU + i] - d22[DT + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[GE + i] - Overall_harmonious_min_perc)))

      // =IF(GX6=0,0,(Q6+(R6-Q6)/(AE6-A_equiv_harmonious_min_perc)*(GX6-A_equiv_harmonious_min_perc)))
      d23[IE + i] = iff(d23[GX + i].isZero, 0.0, (d21[Q + i] + (d21[R + i] - d21[Q + i]) * d23[ddGX + i]))
      ...
      d23[IH + i] = iff(d23[GE + i].isZero, 0.0, min((d22[EP + i] + (d22[EQ + i] - d22[EP + i]) * d23[ddGE + i]) * El_boiler_eff, max(0.0, d23[IE + i] - d23[IF + i])))

      // IF6+IH6-IE6
      d23[II + i] = d23[IF + i] + d23[IH + i] - d23[IE + i]

      // HU=IF(DR6=0,0,IF(GE6=0,MAX(0,DS6-MIN(BESS_cap_ud/BESS_chrg_eff,EL6,MAX(0,EA6/BESS_chrg_eff-EM6-EL6))),DR6+(DS6-DR6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)))
      if d22[DR + i].isZero {
        d23[HU + i] = 0.0
      } else if d23[GE + i].isZero {
        d23[HU + i] = max(0.0, d22[DS + i] - min(BESS_cap_ud / BESS_chrg_eff, d22[EL + i], max(0.0, d22[EA + i] / BESS_chrg_eff - d22[EM + i] - d22[EL + i])))
      } else {
      ...
      d23[HV + i] = iff(d23[GX + i].isZero, 0.0, (d21[O + i] + (d21[P + i] - d21[O + i]) * d23[ddGX + i]))

      // HW=IF(OR(GE6=0,GX6=0),MIN(BESS_cap_ud/BESS_chrg_eff,MAX(0,EA6/BESS_chrg_eff-EM6),EL6+MAX(0,DS6-HU6)),MIN(EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(QI6-Overall_harmonious_min_perc),MAX(0,EA6+E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GX6-A_equiv_harmonious_min_perc)+MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GX6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff))
      d23[HW + i] = iff(
        or(d23[GE + i].isZero, d23[GX + i].isZero), min(BESS_cap_ud / BESS_chrg_eff, max(0.0, d22[EA + i] / BESS_chrg_eff - d22[EM + i]), d22[EL + i] + max(Double.zero, d22[DS + i] - d23[HU + i])),
        min(
          d22[EK + i] + (d22[EL + i] - d22[EK + i]) * d23[ddGE + i],
          max(
            0.0,
            d22[EA + i] + (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGX + i]) + min(d22[ER + i], max(0.0, (d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGX + i]) - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff)
            / BESS_chrg_eff))
      ...
      // HZ=IF(GE6=0,MAX(0,EC6+MAX(0;DS6-HU6)),EB6+(EC6-EB6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc))
      d23[HZ + i] = iff(d23[GE + i].isZero, max(Double.zero, d22[EC + i] + max(Double.zero, d22[DS + i] - d23[HU + i])), d22[EB + i] + (d22[EC + i] - d22[EB + i]) * d23[ddGE + i])
    ...
    for i in 0..<365 { d23[IB + i] = min(iff(d23[GE + i].isZero, 0.0, d22[EH + i] + (d22[EI + i] - d22[EH + i]) * d23[ddGE + i]), max(0.0, -(d23[HZ + i] - d23[HV + i] - d23[HW + i] - d23[HY + i]))) }
    ...
    for i in 0..<365 { d23[IQ + i] = iff(d23[GX + i].isZero, 0.0, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGX + i]) }
    ...
    for i in 0..<365 { d23[IS + i] = iff(or(d23[GE + i].isZero, d23[GX + i].isZero), 0.0, min(d22[ER + i] * El_boiler_eff, max(0.0, d23[IQ + i] - d23[IR + i]))) }
    ...
    for i in 0..<365 { d23[IJ + i] = iff(d23[GX + i].isZero, 0.0, d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGX + i]) }
    ...
    for i in 0..<365 { d23[IM + i] = min(max(0.0, d23[IL + i] + d23[IK + i] + d23[IJ + i] - d23[IN + i]) / BESS_chrg_eff, d22[EM + i]) }

    /// grid input outside of harm op period
    let IO = 31755
    // IO=MIN(DO6+EJ6,MAX(0,-IM6*BESS_chrg_eff-IN6+IJ6+IK6+IL6))
    for i in 0..<365 { d23[IO + i] = min(d22[DO + i] + d22[EJ + i], max(0.0, -d23[IM + i] * BESS_chrg_eff - d23[IN + i] + d23[IJ + i] + d23[IK + i] + d23[IL + i])) }

    /// Balance of electricity outside of harm op period
    let IP = 32120
    // IP=IM6*BESS_chrg_eff+IN6+IO6-IJ6-IK6-IL6
    for i in 0..<365 { d23[IP + i] = d23[IM + i] * BESS_chrg_eff + d23[IN + i] + d23[IO + i] - d23[IJ + i] - d23[IK + i] - d23[IL + i] }
    ...
    /// grid export
    let IV = 34310
    // IV=ROUND(MAX(0,MIN(IC6,IF(GE6=0,DW6,(DV6+(DW6-DV6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(GE6-Overall_harmonious_min_perc)))))+MAX(0,MIN(IP6,IF(OR(GE6=0,GX6=0),0,DX6))),0)
    for i in 0..<365 {
      d23[IV + i] = round(
        max(0.0, min(d23[IC + i], iff(or(d23[GE + i].isZero, d23[GX + i].isZero), d22[DW + i], (d22[DV + i] + (d22[DW + i] - d22[DV + i]) * d23[ddGE + i]))) + min(d23[IP + i], iff(or(d23[GE + i].isZero, d23[GX + i].isZero), 0.0, d22[DX + i]))),
        0)
    }

    /// grid import
    let IW = 34675
    // IW=ROUND(MAX(0,-IC6)+MAX(0,-IP6),0)*EDG_elec_cost_factor+ROUND(IA6+IB6+IO6,0)
    for i in 0..<365 { d23[IW + i] = round(max(Double.zero, -d23[IC + i]) + max(Double.zero, -d23[IP + i]), 0) * EDG_elec_cost_factor + round(d23[IA + i] + d23[IB + i] + d23[IO + i], 0) }

    /// Checksum
    let IX = 35040
    // IX=ROUND(MAX(0,-IC6)+MAX(0,-II6)+MAX(0,-IP6)+MAX(0,-IT6),0)
    for i in 0..<365 {
      // if IX > 1E-13 { print("Checksum error daily 2", i, j, IX) }
      d23[IX + i] = round(max(Double.zero, -d23[IC + i]) + max(Double.zero, -d23[II + i]) + max(Double.zero, -d23[IP + i]) + max(Double.zero, -d23[IT + i]), 0)
    }
    ...
    for i in 0..<365 { d23[JI + i] = iff(d22[DT + i].isZero, Double.zero, iff(d23[HS + i].isZero, d22[DU + i], d22[DT + i] + (d22[DU + i] - d22[DT + i]) * d23[ddHS + i])) }
    ...
    for i in 0..<365 { d23[JM + i] = iff(d23[HS + i].isZero, Double.zero, min((d22[EP + i] + (d22[EQ + i] - d22[EP + i]) * d23[ddHS + i]) * El_boiler_eff, max(0.0, d23[JJ + i] - d23[JK + i]))) }
    ...
    /// el cons for harm op during harm op period
    let IZ = 35405
    // IZ=IF($DR6=,0,IF(HS6=0;MAX(0,$DS6-MIN(BESS_cap_ud/BESS_chrg_eff;$EL6;MAX(0;$EA6/BESS_chrg_eff-$EL6-$EM6))),$DR6+($DS6-$DR6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(HS6-Overall_harmonious_min_perc)))
    for i in 0..<365 {
      d23[IZ + i] = iff(
        d22[DR + i].isZero, Double.zero, iff(d23[HS + i].isZero, max(Double.zero, d22[DS + i] - min(BESS_cap_ud / BESS_chrg_eff, d22[EL + i], max(Double.zero, d22[EA + i] / BESS_chrg_eff - d22[EL + i] - d22[EM + i]))), d22[DR + i] + (d22[DS + i] - d22[DR + i]) * d23[ddHS + i]))
    }

    /// el cons for night prep during harm op period
    let JA = 35770
    // JA=IF(GZ6=0,0,(O6+(P6-O6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)))
    for i in 0..<365 { d23[JA + i] = iff(d23[GZ + i].isZero, Double.zero, (d21[O + i] + (d21[P + i] - d21[O + i]) * d23[ddGZ + i])) }

    /// el cons for BESS charging during harm op period
    let JB = 36135
    // JB=IF(OR(GZ6=0,HS6=0),MIN(BESS_cap_ud/BESS_chrg_eff,MAX(0,EA6/BESS_chrg_eff-EM6),EL6+MAX(0,$DS6-IZ6));MIN(EK6+(EL6-EK6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(RW6-Overall_harmonious_min_perc),MAX(0,EA6+E6+(F6-E6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)+MIN(ER6,MAX(0,G6+(H6-G6)/(AE6-A_equiv_harmonious_min_perc)*(GZ6-A_equiv_harmonious_min_perc)-EG6)/El_boiler_eff)-EJ6-EM6*BESS_chrg_eff)/BESS_chrg_eff))
    for i in 0..<365 {
      d23[JB + i] = iff(
        or(d23[HS + i].isZero, d23[GZ + i].isZero), min(BESS_cap_ud / BESS_chrg_eff, max(Double.zero, d22[EA + i] / BESS_chrg_eff - d22[EM + i]), d22[EL + i] + max(Double.zero, d22[DS + i] - d23[IZ + i])),
        min(
          d22[EK + i] + (d22[EL + i] - d22[EK + i]) * d23[ddHS + i],
          max(
            0.0,
            d22[EA + i] + (d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i]) + min(d22[ER + i], max(Double.zero, (d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i]) - d22[EG + i]) / El_boiler_eff) - d22[EJ + i] - d22[EM + i] * BESS_chrg_eff)
            / BESS_chrg_eff))
    }
    ...
    /// PV available after harm op during harm op period
    let JE = 37230
    // JE=IF(HS6=0,MAX(0,EC6+MAX(0,DS6-IZ6)),EB6+(EC6-EB6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(HS6-Overall_harmonious_min_perc))
    for i in 0..<365 {
      d23[JE + i] = iff(
        d23[HS + i].isZero, max(Double.zero, d22[EC + i] + max(Double.zero, d22[DS + i] - d23[IZ + i])),
        d22[EB + i] + (d22[EC + i] - d22[EB + i]) / (Overall_harmonious_max_perc - Overall_harmonious_min_perc) * (d23[HS + i] - Overall_harmonious_min_perc))
    }
    ...
    for i in 0..<365 { d23[JG + i] = min(iff(d23[HS + i].isZero, Double.zero, d22[EH + i] + (d22[EI + i] - d22[EH + i]) * d23[ddHS + i]), max(Double.zero, -(d23[JE + i] - d23[JA + i] - d23[JB + i] - d23[JD + i]))) }
    ...
    for i in 0..<365 { d23[JV + i] = iff(d23[GZ + i].isZero, Double.zero, d21[G + i] + (d21[H + i] - d21[G + i]) * d23[ddGZ + i]) }
    ...
    for i in 0..<365 { d23[JX + i] = iff(or(d23[HS + i].isZero, d23[GZ + i].isZero), Double.zero, min(d22[ER + i] * El_boiler_eff, max(Double.zero, d23[JV + i] - d22[EG + i]))) }
    ...
    for i in 0..<365 { d23[JO + i] = iff(d23[GZ + i].isZero, Double.zero, d21[E + i] + (d21[F + i] - d21[E + i]) * d23[ddGZ + i]) }
    ...
    for i in 0..<365 {
      ...
      d23[JR + i] = min(max(Double.zero, d22[EA + i] + d23[JP + i] + d23[JO + i] - d23[JS + i]) / BESS_chrg_eff, d22[EM + i])
    }

    /// grid input outside of harm op period
    let JT = 42705
    // JT=MIN(DO6+EJ6,MAX(0,-JR6*BESS_chrg_eff-JS6+JO6+JP6+JQ6))
    for i in 0..<365 { d23[JT + i] = min(d22[DO + i] + d22[EJ + i], max(Double.zero, -d23[JR + i] * BESS_chrg_eff - d23[JS + i] + d23[JO + i] + d23[JP + i] + d22[EA + i])) }
    ...
    /// grid export
    let KA = 45260
    // KA=ROUND(MAX(0,MIN(JH6,IF(HS6=0,DW6,(DV6+(DW6-DV6)/(Overall_harmonious_max_perc-Overall_harmonious_min_perc)*(HS6-Overall_harmonious_min_perc)))))+MAX(0,MIN(JU6,IF(OR(HS6=0,GZ6=0),0,DX6))),0)
    for i in 0..<365 {
      d23[KA + i] = round(
        max(0.0, min(d23[JH + i], iff(or(d23[HS + i].isZero, d23[GZ + i].isZero), d22[DW + i], (d22[DV + i] + (d22[DW + i] - d22[DV + i]) * d23[ddHS + i]))))
          + max(0.0, min(d23[JU + i], iff(or(d23[HS + i].isZero, d23[GZ + i].isZero), Double.zero, d22[DX + i]))), 0)
    }

    /// grid import
    let KB = 45625
    // KB=ROUND(MAX(0,-JH6)+MAX(0,-JU6),0)*EDG_elec_cost_factor+ROUND(JF6+JG6+JT6,0)
    for i in 0..<365 { d23[KB + i] = round(max(Double.zero, -d23[JH + i]) + max(Double.zero, -d23[JU + i]),0) * EDG_elec_cost_factor + round(d23[JF + i] + d23[JG + i] + d23[JT + i], 0) }
  }
}
