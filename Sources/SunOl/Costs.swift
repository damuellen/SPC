  ...
  public func invest(_ model: TunOl) -> [Double]
   {
    ...
    let Assembly_hall = 12_000_000.0 + (-132.967033 * Double(model.CSP_loop_nr_ud) ** 2 + 62978.02 * Double(model.CSP_loop_nr_ud) + -2.32831E-10)
    ...
    let Hydrogen_Storage_cost = model.Hydrogen_storage_cap_ud * Costs.Hydrogen_storage.coeff + 0.0
    ...
    let Electrical_boiler_cost = model.El_boiler_cap_ud > 0 ?
      (Costs.Electrical_boiler_capacity.coeff * Costs.Electrical_boiler_capacity.basis * (model.El_boiler_cap_ud / Costs.Electrical_boiler_capacity.basis)
      ** Costs.Electrical_boiler_capacity.exp) : 0
    ...
    let Substation_cost =
      Costs.Substation_capacity.coeff * Costs.Substation_capacity.basis * (max(model.Grid_export_max_ud, model.Grid_import_max_ud) / Costs.Substation_capacity.basis) ** Costs.Substation_capacity.exp

    let CSP_O_M_Cost = (11.3333 / 3 * 1 / 3 * 1000 * 1000) + (0.00606061 / 3 * 1 / 3 * 1000 * 1000) * model.CSP_loop_nr_ud
    let PV_O_M_Cost = (11.3333 * 1000 * 1000) + 0 * model.PV_DC_cap_ud
    ...
    // let LCoM =
    //   (FCR * Total_CAPEX + Total_OPEX + elec_from_grid_sum * BUY * 1000 - elec_to_grid_sum * SELL * 1000) / meth_produced_MTPH_sum
